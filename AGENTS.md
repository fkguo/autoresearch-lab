...
- **主协调线程与 lane `branch/worktree` 必须分离**：主协调线程默认固定在根仓 `/Users/fkg/Coding/Agents/autoresearch-lab` 的 `main` worktree；允许通过 shell 创建、删除或维护 lane `worktree` / `branch`，但这些 lane 仅是独立执行面，不得改变主协调线程的 `branch/worktree` 关联。
