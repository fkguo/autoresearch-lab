# Review Bundle (v14): idea-generator architecture spec + schemas + RPC
...
- `campaign.topup`：`running|paused|early_stopped|exhausted → (same state)`；但 **若当前为 `exhausted`，topup 成功后必须使 status 变为 `running`**（避免“加了预算仍无法继续”的死状态）
...

{
  ...
  "methods": [
    ...
    },
    {
      ...
      "description": "Side-effecting. Transitions the campaign into a terminal completed state. Read-only methods remain available after completion.",
      "result": { "name": "campaign_mutation_result", "schema": { "$ref": "./campaign_topup_result_v1.schema.json" } },
      "errors": [{ "code": -32003, "message": "campaign_not_found" }]
    },
    {
