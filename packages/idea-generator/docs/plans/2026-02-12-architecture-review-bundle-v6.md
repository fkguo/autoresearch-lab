# Review Bundle (v6): idea-generator architecture spec + schemas + RPC

Please review the bundle below. Focus on:
- Are contracts still machine-enforceable? (JSON Schema + OpenRPC)
...
- Are the most recent portability/observability patches coherent? (eval.run node_not_found; promotion grounding summary; IdeaNode updated_at; IslandState team_policy_id; BudgetSnapshot oneOf; search.step n_steps default)
