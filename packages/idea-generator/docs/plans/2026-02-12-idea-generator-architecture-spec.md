...
> 日期：2026-02-12  
> 目标：以 **HEP 作为首个 domain-pack 落地方向**，建立一个可扩展到理论物理其它分支的 `idea-generator` 架构规格（可实现、可测试、可演进）。
...
1. **可扩展**：以 HEP（hep-ph/hep-th/nucl-th）作为首个落地 pack；未来扩展到凝聚态/天体/数学物理时不改 core，只加 pack/plugin。
...
- **最小方法集（v1）**：
  ...
  - `eval.run`
  - `rank.compute`
- **错误码约定**：至少包含 `budget_exhausted` / `schema_validation_failed` / `invalid_charter` / `grounding_audit_failed` / `reduction_audit_failed` / `insufficient_eval_data` / `campaign_not_found` / `campaign_not_active` / `node_not_found` / `node_not_in_campaign`
...
（把科学史/科学哲学转成可执行算子族；首个 domain pack 可先实现其中 4–6 个）
...
#### 3.4.2 角色建议（首个理论研究 domain pack 的最小社区）
...
2. `IdeaCard`（低温约束 + schema 校验）：
   ...
   - `candidate_formalisms[]`（可选的 run-local 方法/形式提示；如存在仅作为局部上下文保留，不得作为 promote 到 C2 的 mandatory gate）
...
v0.2 建议把扩展点收敛为 6 类（先由首个 domain pack 落地验证，再抽取接口）：
...
**重要：首个 domain pack 是 HEP ≠ core 只服务 HEP（跨学科方法迁移必须预留接口）**
...
**method hint / compiler boundary（避免把 worldview registry 提升为公共 authority）**：
- DomainPack 可以声明 provider-local 的 compiler / validator / execution helper，但不得要求 repo-level canonical formalism registry。
- `candidate_formalisms[]` 若存在，只能作为 user/project/run-local 的非 authoritative 方法提示；未知值不得阻塞 `node.promote` 或 C2 handoff。
- `node.promote` 的主线门禁应继续围绕 schema 完整性、grounding audit、reduction audit 与 evidence/provenance，而不是 registry membership。
...
## 8. 与现有 autoresearch 生态圈集成

### 8.1 输入（seed sources）

- C1 gaps（系统性缺口）
- KB priors（已有笔记/失败记录/方法痕迹）
- 当前 domain pack 的 structured evidence / tensions（例如 HEP 中的 PDG/HEPData）
...
4. 最小首个 DomainPack（当前默认 HEP；实现 2–3 个 operator + novelty/grounding MVP）
