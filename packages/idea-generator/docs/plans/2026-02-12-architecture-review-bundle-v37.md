# Review Bundle (v37): rank.compute no_scorecards + promotion reduction summary constraints + audit provenance
Please review the bundle below. Focus on:
- `rank.compute` error semantics are explicit for `no_scorecards` and effective-dimension counting
- `promotion_result_v1` is now self-auditable for reductions (`has_reduction_report` + required conditional `reduction_audit_summary`)
- `reduction_audit_v1` is self-contained for provenance (`abstract_problem` echoed)
- Contract completeness: **all** `schemas/*.schema.json` are included so OpenRPC `$ref` closure can be verified
...
6. **归约审计（reduction_audit，条件性强约束）**：当 `IdeaNode.reduction_report != null`（通常来自 `ProblemReduction`/`TechniqueTransplant`）时，`node.promote` 的成功条件**额外**要求 `reduction_audit != null` 且 `reduction_audit.status == pass`。若 `reduction_audit == null`，必须阻塞晋升并返回 `reduction_audit_failed`（`error.data.reason=reduction_audit_missing`）。`partial/fail` 一律阻塞晋升（返回 `reduction_audit_failed`）。审计最小检查项：
   - `assumptions`：归约所需的每条前提是否被验证或标注为 `pending_verification`（全部 pending ⇒ `partial`；任一 violated ⇒ `fail`）
   - `toy_check_result: pass | fail | skipped`（`skipped` 仅在 `Coder/Checker` 角色不可用或预算熔断时允许，且必须标注 `skip_reason`）
...
$p_{t+1} = \arg\max_p \left[ \langle p, Q \rangle - \frac{1}{\eta} \mathrm{KL}(p \| p_t) \right]$
...

{
  ...
  "methods": [
    ...
    },
    {
      ...
      "description": "Side-effecting. Only permitted when campaign status is running; otherwise campaign_not_active. Filter is applied within campaign scope. For method=pareto, the filter MUST resolve to >= 1 node (a single node is allowed and yields rank=1). Dimension selection: let observed_keys be the set of distinct keys observed in scorecards[].scores across the resolved node set (implementations MAY ignore scorecards with status=failed and MAY intersect with EvaluatorConfig.dimensions). If dimensions is provided, effective_dimensions = dimensions ∩ observed_keys; otherwise effective_dimensions = observed_keys. If observed_keys is empty (i.e., eval.run has never been called for the resolved set, or all scorecards are missing/failed), the engine MUST return insufficient_eval_data (-32013) with error.data.reason=no_scorecards and MUST NOT write ranking artifacts. For method=pareto, effective dimension count MUST be >= 2; otherwise the engine MUST return insufficient_eval_data (-32013) with error.data.reason=insufficient_dimensions and MUST NOT write ranking artifacts. For method=elo, the filter MUST resolve to >= 2 nodes. If the resolved set is too small, or evaluation data is insufficient, the engine MUST return insufficient_eval_data (-32013) (reason=insufficient_nodes or no_scorecards) and MUST NOT write ranking artifacts.",
      "result": {
        "name": "ranking_result",
        "schema": {
