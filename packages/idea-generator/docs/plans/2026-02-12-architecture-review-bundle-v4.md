# Review Bundle (v4): idea-generator architecture spec + schemas + RPC

Please review the bundle below. Focus on:
- Are contracts now machine-enforceable? (JSON Schema + OpenRPC; OpenRPC now references standalone schemas via $ref)
- Are the previous blockers resolved? (node_id vs idea_id ambiguity; pagination; eval/run campaign scoping; insufficient_eval_data; schema SSOT)
- Does the Team/Role community model remain modular and compatible with hepar control-plane?
...
- **最小方法集（v1）**：
  ...
  - `eval.run`
  - `rank.compute`
- **错误码约定**：至少包含 `budget_exhausted` / `schema_validation_failed` / `grounding_audit_failed` / `formalism_not_in_registry` / `insufficient_eval_data` / `campaign_not_found` / `node_not_found`
  ...
  "methods": [
    {
      ...
      "params": [
        ...
        { "name": "formalism_registry", "schema": { "$ref": "#/components/schemas/FormalismRegistryV1" } },
    ...
    },
    {
      ...
      "params": [
        ...
        { "name": "n_steps", "schema": { "type": "integer", "minimum": 1 } },
        { "name": "idempotency_key", "schema": { "type": "string", "minLength": 1 } }
      ],
    ...
    },
    {
      ...
      "paramStructure": "by-name",
      "params": [{ "name": "node_id", "schema": { "type": "string", "format": "uuid" }, "required": true }],
    ...
    {
      "name": "node.promote",
      "summary": "Promote a node to a C2 handoff artifact (after schema + grounding + formalism gates).",
      "paramStructure": "by-name",
      "params": [{ "name": "node_id", "schema": { "type": "string", "format": "uuid" }, "required": true }],
...
--- BEGIN: schemas/campaign_init_result_v1.schema.json ---
{
  ...
  "properties": {
    ...
    "island_states": { "type": "array", "items": { "$ref": "island_state_v1.schema.json" } }
  },
  "additionalProperties": false
}
...
--- BEGIN: schemas/promotion_result_v1.schema.json ---
{
  ...
  "type": "object",
  "required": ["node_id", "idea_id", "handoff_artifact_ref", "formalism_check", "grounding_audit_summary"],
  "properties": {
    "node_id": { "type": "string", "format": "uuid" },
    "idea_id": { "type": "string", "format": "uuid" },
