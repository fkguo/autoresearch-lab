# Review Bundle (v2): idea-generator architecture spec + schemas + RPC

Please review the bundle below. Focus on:
- Are contracts now machine-enforceable? (JSON Schema + OpenRPC)
- Are the remaining blockers resolved? (state machine, grounding audit, method compiler contract)
- Are we still minimal-but-extensible for HEP-first → broader theory physics?
...
- **最小方法集（v1）**：
  ...
  - `eval.run`
  - `rank.compute`
- **错误码约定**：至少包含 `budget_exhausted` / `schema_validation_failed` / `grounding_audit_failed` / `campaign_not_found` / `idea_not_found`
...
--- BEGIN: schemas/rationale_draft_v1.schema.json ---
{
  ...
  "properties": {
    ...
    "kill_criteria": { "type": "array", "items": { "type": "string", "minLength": 1 } },
...
--- BEGIN: schemas/idea_card_v1.schema.json ---
{
  ...
  "properties": {
    ...
    "candidate_formalisms": { "type": "array", "minItems": 1, "items": { "type": "string", "minLength": 1 } },
    "minimal_compute_plan": {
      "type": "array",
      "minItems": 1,
      "items": {
...
--- BEGIN: schemas/idea_core_rpc_v1.openrpc.json ---
{
  ...
  "info": {
    ...
    "version": "1.0.0",
    "description": "JSON-RPC interface between the idea-generator adapter (hepar skill) and the standalone idea-core engine."
  },
  "methods": [
    {
      "name": "campaign.init",
      ...
      "params": [
        { "name": "charter", "schema": { "type": "object" } },
        { "name": "seed_pack", "schema": { "type": "object" } },
        { "name": "budget", "schema": { "type": "object" } }
      ],
      "result": {
        "name": "campaign_init_result",
        "schema": { "type": "object", "required": ["campaign_id"], "properties": { "campaign_id": { "type": "string" } } }
      },
      "errors": [
    ...
    },
    {
      ...
      "params": [{ "name": "campaign_id", "schema": { "type": "string" } }],
      "result": { "name": "campaign_status", "schema": { "type": "object" } },
    ...
    },
    {
      ...
      "params": [
        { "name": "campaign_id", "schema": { "type": "string" } },
        { "name": "n_steps", "schema": { "type": "integer", "minimum": 1 }, "required": false }
      ],
      "result": { "name": "search_step_result", "schema": { "type": "object" } },
      "errors": [
    ...
    },
    {
      ...
      "result": { "name": "idea_node", "schema": { "$ref": "idea_node_v1.schema.json" } },
    ...
    },
    {
      ...
      "params": [
        { "name": "campaign_id", "schema": { "type": "string" } },
        { "name": "filter", "schema": { "type": "object" }, "required": false }
      ],
      "result": {
        "name": "idea_list",
        "schema": { "type": "object", "properties": { "ideas": { "type": "array", "items": { "$ref": "idea_node_v1.schema.json" } } } }
      },
      "errors": [{ "code": -32003, "message": "campaign_not_found" }]
    },
    {
      "name": "idea.promote",
      "summary": "Promote an idea to a C2 handoff artifact (after schema + grounding gates).",
      "params": [{ "name": "idea_id", "schema": { "type": "string", "format": "uuid" } }],
      "result": { "name": "promotion_result", "schema": { "type": "object" } },
      "errors": [
        { "code": -32002, "message": "schema_validation_failed" },
    ...
    },
    {
      ...
      "params": [
        { "name": "idea_ids", "schema": { "type": "array", "items": { "type": "string", "format": "uuid" } } },
        { "name": "evaluator_config", "schema": { "type": "object" } }
      ],
      "result": { "name": "eval_result", "schema": { "type": "object" } },
    ...
    },
    {
      ...
      "result": { "name": "ranking_result", "schema": { "type": "object" } },
      "errors": [{ "code": -32003, "message": "campaign_not_found" }]
    }
  ]
}
