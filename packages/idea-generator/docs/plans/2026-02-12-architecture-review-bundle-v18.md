# Review Bundle (v18): idea-generator architecture spec + schemas + RPC
...

{
  ...
  "methods": [
    ...
    },
    {
      ...
      "description": "Side-effecting. Callable when campaign status is paused|early_stopped|exhausted. Transitions paused|early_stopped → running. If status is exhausted (or budget is exhausted), the engine MUST return budget_exhausted (-32001) without changing state (caller should topup first). If campaign is completed, returns campaign_not_active (-32015).",
      "result": { "name": "campaign_mutation_result", "schema": { "$ref": "./campaign_mutation_result_v1.schema.json" } },
      "errors": [
        { "code": -32001, "message": "budget_exhausted" },
        { "code": -32003, "message": "campaign_not_found" },
