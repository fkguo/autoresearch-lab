# Review Bundle (v35): full schema closure + reduction/distributor gates
Please review the bundle below. Focus on:
- Cross-disciplinary: `ProblemReduction` workflow is contract-backed (`reduction_report` + machine-checkable `reduction_audit` + promotion gate)
- Distributor: campaign-declared `factorization` + auditable artifacts + factorized configs enumerate action space IDs
