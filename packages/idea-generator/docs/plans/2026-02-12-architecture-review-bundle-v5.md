# Review Bundle (v5): idea-generator architecture spec + schemas + RPC

Please review the bundle below. Focus on:
- Are contracts still machine-enforceable? (JSON Schema + OpenRPC; schema portability changes: oneOf(null,object), required step_id, campaign_id echo)
- Are the previous blockers still resolved? (node_id vs idea_id ambiguity; pagination; eval.run campaign scoping; insufficient_eval_data; schema SSOT)
- Are new additions consistent and modular? (Distributor formula details; phenotype profiling metrics; OpenClaw/OpenCode runtime mapping remains adapter-friendly)
