...
- 任务 ID 使用语义前缀（`RES` / `CTR` / `CORE` / `HEP` / `INT` / `QL`），不再沿用需要历史上下文才能理解的 `W*` 前缀。
...
- **Top next**：
  1) `HEP-02`：补齐 operator families 的通用设计与验收口径
  2) `QL-06`：把 clean-room 多评审 / debate trigger 收敛为通用 gate
...
|---|---|---|---|---|---|---|
| RES-01 | Research | IdeaSearch / OpenClaw 证据补强 | 深读文档 + 可迁移机制清单 | DONE | M0 | 见 `docs/plans/2026-02-12-ideasearch-openclaw-deep-dive.md` |
| RES-03 | Research | OpenCode server / permission 映射 | runtime adapter 端点族清单 | DONE | M0 | 见 `docs/plans/2026-02-12-opencode-hepar-compatibility.md` |
| RES-04 | Research | 科学发现 → 可执行算子库 | operator families 文档 | DONE | M0 | 见 `docs/plans/2026-02-12-executable-discovery-operators.md` |
| RES-06 | Research | Distributor 策略设计 | allocator / audit 建议 | DONE | M0 | 见 `docs/plans/2026-02-12-statphys-distributor-policies.md` |
| RES-07 | Research | Bandit / allocator alternatives | allocator 方案比较 | DONE | M0 | 见 `docs/plans/2026-02-12-bandit-distributor-alternatives.md` |
| CTR-00 | Contracts | 契约补丁与冻结 | schemas + OpenRPC 更新 | DONE | M0 | Authority 仍在 `schemas/` |
| CTR-01 | Contracts | Schema/OpenRPC 校验命令 | validate script / CI hook | DONE | M1 | 落地于 `packages/idea-core` |
| CTR-02 | Contracts | OpenRPC 打包 / 解引用 | bundled OpenRPC | DONE | CTR-01 | 落地于 `packages/idea-core` |
| CTR-03 | Contracts | Reduction / distributor 契约补丁 | schemas + spec 更新 | DONE | M0 | Authority 仍在 `schemas/` |
| CTR-04 | Contracts | non-blocking contract amendments | schemas + spec 更新 | DONE | M0 | 已吸收入当前 authority |
| CORE-01 | Core | IdeaStore + 索引/分页 | store + tests | DONE | M2.1 | 落地于 `packages/idea-core` |
| CORE-02 | Core | JSON-RPC server（stdio） | rpc server + tests | DONE | M2.3 | 落地于 `packages/idea-core` |
| CORE-03 | Core | Budget circuit breaker | budget module + tests | DONE | M2.4 | 落地于 `packages/idea-core` |
| CORE-04 | Core | Multi-Island loop | island loop + tests | DONE | M2.5 | 落地于 `packages/idea-core` |
| CORE-05 | Core | Eval pipeline（MVP） | scorecards + writeback | DONE | M2.9 | 落地于 `packages/idea-core` |
| CORE-06 | Core | Ranking（Pareto/Elo） | ranking artifact | DONE | M2.10 | 落地于 `packages/idea-core` |
| CORE-07 | Core | Promote（handoff） | handoff artifact | DONE | M2.11 | 落地于 `packages/idea-core` |
...
| HEP-03A | HEP Pack | novelty delta discipline | schema-validated scorecards | DONE | HEP-03 | 落地于 `packages/idea-core` |
| HEP-04 | HEP Pack | constraints / validators | validator outputs | DONE | HEP-03 | 落地于 `packages/idea-core` |
| HEP-05 | HEP Pack | compute plan rubric | resource rubric | DONE | HEP-04 | 落地于 `packages/idea-core` |
| INT-01 | Integration | WorkOrder / WorkResult / TeamPlan | artifacts + ledger | DONE | M4 | 落地于 `packages/idea-core` |
