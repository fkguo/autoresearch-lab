# Review Bundle (v15): idea-generator architecture spec + schemas + RPC
...
- `campaign.pause`：`running|early_stopped → paused`（把“算法早停”冻结为“人工暂停”）
...
- `campaign.topup`：`running|paused|early_stopped|exhausted → (same state)`；若当前为 `exhausted`，topup 后 **若预算已不再耗尽（各受限维度 remaining > 0）则转为 `running`，否则保持 `exhausted`**（避免“加了预算仍无法继续”的死状态，也避免“只 topup 一维但仍不可继续”的逻辑矛盾）
...

{
  ...
  "methods": [
    ...
    },
    {
      ...
      "description": "Monotonic, additive budget top-up. Permitted when campaign status is running|paused|early_stopped|exhausted. If status is exhausted, successful topup MUST transition campaign status to running only if the campaign is no longer budget-exhausted after applying the top-up; otherwise it remains exhausted. If the campaign is completed, the engine MUST reject the request with campaign_not_active.",
    ...
    },
    {
      ...
      "description": "Side-effecting. Permitted when campaign status is running or early_stopped (running|early_stopped → paused). Otherwise campaign_not_active.",
      "result": { "name": "campaign_mutation_result", "schema": { "$ref": "./campaign_topup_result_v1.schema.json" } },
      "errors": [
        { "code": -32003, "message": "campaign_not_found" },
        { "code": -32015, "message": "campaign_not_active" }
    ...
    },
    {
      ...
      "description": "Side-effecting. Permitted when campaign status is paused or early_stopped. If budget is exhausted, returns budget_exhausted. If campaign is completed, returns campaign_not_active.",
      "result": { "name": "campaign_mutation_result", "schema": { "$ref": "./campaign_topup_result_v1.schema.json" } },
      "errors": [
        { "code": -32001, "message": "budget_exhausted" },
        { "code": -32003, "message": "campaign_not_found" },
    ...
    },
    {
      ...
      "description": "Side-effecting. Permitted when status is running|paused|early_stopped|exhausted. Transitions the campaign into a terminal completed state. If already completed, the engine SHOULD treat this as a no-op (but still idempotent). Read-only methods remain available after completion.",
      "result": { "name": "campaign_mutation_result", "schema": { "$ref": "./campaign_topup_result_v1.schema.json" } },
      "errors": [{ "code": -32003, "message": "campaign_not_found" }]
    },
    {
    ...
    },
    {
      ...
      "description": "Read-only. Permitted in any campaign status. Results are always scoped to the given campaign; filters that do not match the campaign return empty results.",
