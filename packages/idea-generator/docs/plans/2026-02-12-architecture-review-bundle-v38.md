# Review Bundle (v38): rank.compute failure reasons are now MUST + deterministic
Please review the bundle below. Focus on:
- `rank.compute` now has a MUST-level decision rule for `no_scorecards` (ignore failed scorecards) vs `insufficient_dimensions` vs `insufficient_nodes`
- Promotion/reduction provenance remains machine-enforceable (`has_reduction_report` + conditional summaries; handoff includes reduction artifacts)
