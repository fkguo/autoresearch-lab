# Review Bundle (v12): idea-generator architecture spec + schemas + RPC
...
- `schemas/campaign_topup_result_v1.schema.json`
...
1. **Campaign scoping MUST**：涉及 `campaign_id` 的 RPC，engine 必须保证严格的 campaign 隔离（防止跨 campaign 污染/泄漏）。
   ...
   - 对“列表/过滤”风格的 RPC（`node.list` / `rank.compute` 的 `filter`），引擎必须把结果 **天然限定在该 campaign 内**；若 filter 中的 `idea_id/node_id` 不匹配该 campaign，应返回空结果（而不是报错），以保持 list 语义简单且可组合。
2. **Idempotency MUST（side-effecting calls）**：对会落盘/写 artifact/更新节点的 RPC（`campaign.init`/`campaign.topup`/`search.step`/`eval.run`/`rank.compute`/`node.promote`），adapter **必须**提供 `idempotency_key`（OpenRPC 要求）；engine 必须按 `(method, campaign_id?, idempotency_key)` 去重，确保重试不会产生重复产物或非确定性写入（`campaign.init` 无 `campaign_id`，因此该项为空；参考 OpenClaw gateway protocol 的同类纪律）。

   **Idempotency replay 规则（必须可测试）**：
   - 适用范围：`campaign.init` / `campaign.topup` / `search.step` / `eval.run` / `rank.compute` / `node.promote`（所有 side-effecting methods）。
...

{
  ...
  "methods": [
    ...
    },
    {
      ...
      "params": [
        ...
        { "name": "topup", "schema": { "$ref": "#/components/schemas/BudgetTopUpV1" }, "required": true },
        { "name": "idempotency_key", "schema": { "type": "string", "minLength": 1 }, "required": true }
      ],
      ...
      "result": { "name": "campaign_topup_result", "schema": { "$ref": "#/components/schemas/CampaignTopUpResultV1" } },
      "errors": [
        { "code": -32002, "message": "schema_validation_failed" },
    ...
    },
    {
      ...
      "description": "Atomicity: if any node_id in node_ids is not in the specified campaign, the engine MUST return node_not_in_campaign (-32014) and perform no partial writes/mutations.",
    ...
    },
    {
      ...
      "params": [
        ...
        { "name": "filter", "schema": { "$ref": "#/components/schemas/IdeaListFilterV1" } },
        {
          ...
          "required": false,
          "description": "Required when method=elo. MUST bound tournament cost and ensure deterministic matchups. Engine MUST return schema_validation_failed (-32002) if method=elo and elo_config is absent."
        },
        { "name": "idempotency_key", "schema": { "type": "string", "minLength": 1 }, "required": true }
      ],
  ...
  "components": {
    "schemas": {
      ...
      "BudgetEnvelopeV1": { "$ref": "./budget_envelope_v1.schema.json" },
      "BudgetLimitV1": { "$ref": "./budget_limit_v1.schema.json" },
      "BudgetTopUpV1": { "$ref": "./budget_topup_v1.schema.json" },
      "IdempotencyMetaV1": { "$ref": "./idempotency_meta_v1.schema.json" },
      "CampaignInitResultV1": { "$ref": "./campaign_init_result_v1.schema.json" },
      "CampaignStatusV1": { "$ref": "./campaign_status_v1.schema.json" },
      "CampaignTopUpResultV1": { "$ref": "./campaign_topup_result_v1.schema.json" },
      "SearchStepResultV1": { "$ref": "./search_step_result_v1.schema.json" },
      "IdeaNodeV1": { "$ref": "./idea_node_v1.schema.json" },
      "IdeaListFilterV1": { "$ref": "./idea_list_filter_v1.schema.json" },
      "NodeListResultV1": { "$ref": "./node_list_result_v1.schema.json" },
      "EvaluatorConfigV1": { "$ref": "./evaluator_config_v1.schema.json" },
      "EvalResultV1": { "$ref": "./eval_result_v1.schema.json" },
      "EloConfigV1": { "$ref": "./elo_config_v1.schema.json" },
      "RankingResultV1": { "$ref": "./ranking_result_v1.schema.json" },
      "PromotionResultV1": { "$ref": "./promotion_result_v1.schema.json" },
      "FormalismRegistryV1": { "$ref": "./formalism_registry_v1.schema.json" }
    }
  }
}
...

{
  ...
  "type": "object",
  "required": ["campaign_id", "created_at", "budget_snapshot", "island_states", "idempotency"],
  "properties": {
...
--- BEGIN: schemas/campaign_topup_result_v1.schema.json ---

{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "campaign_topup_result_v1.schema.json",
  "title": "CampaignTopUpResult v1",
  "description": "Result of campaign.topup (monotonic budget increase), including idempotency metadata.",
  "type": "object",
...

{
  ...
  "type": "object",
  "required": ["campaign_id", "method", "ranked_nodes", "budget_snapshot", "idempotency"],
  "properties": {
    "campaign_id": { "type": "string", "format": "uuid" },
