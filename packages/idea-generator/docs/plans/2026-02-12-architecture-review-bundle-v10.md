# Review Bundle (v10): idea-generator architecture spec + schemas + RPC
...
2. **Idempotency MUST（side-effecting calls）**：对会落盘/写 artifact/更新节点的 RPC（`campaign.init`/`campaign.topup`/`search.step`/`eval.run`/`rank.compute`/`node.promote`），adapter **必须**提供 `idempotency_key`（OpenRPC 要求）；engine 必须按 `(method, campaign_id?, idempotency_key)` 去重，确保重试不会产生重复产物或非确定性写入（`campaign.init` 无 `campaign_id`，因此该项为空；参考 OpenClaw gateway protocol 的同类纪律）。
   ...
   **Idempotency replay 规则（必须可测试）**：
   ...
   - Duplicate hit：对同一 `(method, campaign_id, idempotency_key)`（`campaign.init` 为 `(method, idempotency_key)`），engine **必须**原样返回首次调用的响应（包含成功 result 或 error）。
...

{
    ...
    },
    {
      ...
      "params": [
        ...
        { "name": "filter", "schema": { "$ref": "#/components/schemas/IdeaListFilterV1" } },
        {
          ...
          "required": false,
          "description": "Optional configuration for method=elo. MUST bound tournament cost and ensure deterministic matchups."
        },
        { "name": "idempotency_key", "schema": { "type": "string", "minLength": 1 }, "required": true }
      ],
  ...
  "components": {
    "schemas": {
      ...
      "BudgetLimitV1": { "allOf": [{ "$ref": "budget_limit_v1.schema.json" }] },
      "BudgetTopUpV1": { "allOf": [{ "$ref": "budget_topup_v1.schema.json" }] },
      "EloConfigV1": { "allOf": [{ "$ref": "elo_config_v1.schema.json" }] },
