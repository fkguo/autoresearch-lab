# Review Bundle (v7): idea-generator architecture spec + schemas + RPC

Please review the bundle below. Focus on:
- Are contracts still machine-enforceable? (JSON Schema + OpenRPC)
- Are the previous blockers truly resolved? (pagination cursor semantics; campaign scoping; budget consistency; schema SSOT)
- Are side-effecting RPC semantics robust? (idempotency_key discipline; auditability)
...
1. **Campaign scoping MUST**：凡是入参包含 `campaign_id` 的 RPC，engine 必须验证所有涉及的 `node_id/node_ids` 都属于该 campaign；否则返回 `node_not_in_campaign`（防止跨 campaign 污染）。`node.promote` 必须显式携带 `campaign_id` 以便在契约层可验证该约束。
2. **Idempotency MUST（side-effecting calls）**：对会落盘/写 artifact/更新节点的 RPC（`campaign.init`/`search.step`/`eval.run`/`rank.compute`/`node.promote`），adapter 应提供 `idempotency_key`；engine 必须按 `(method, campaign_id, idempotency_key)` 去重，确保重试不会产生重复产物或非确定性写入（参考 OpenClaw gateway protocol 的同类纪律）。
