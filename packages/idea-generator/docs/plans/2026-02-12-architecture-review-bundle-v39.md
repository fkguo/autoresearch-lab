# Review Bundle (v39): non-blocking contract improvements for auditability + usability
Please review the bundle below. Focus on:
- `reduction_audit_v1` now classifies mixed satisfied+pending assumptions (and toy-check skipped) as `partial` (promotion-blocking)
- `ranking_result_v1` now records `effective_dimensions` + `scorecards_artifact_ref` for deterministic audit/replay
- `idea_list_filter_v1` adds `has_eval_info` for common adapter-side intent
- `idea_scorecards_v1` clarifies `status` semantics and makes `failed` scorecards explicitly non-usable (`scores` empty)
- `distributor_event_v1` optionally echoes `rng_alg` for self-contained replay
- OpenRPC bumps to v1.9.6 and clarifies `rank.compute` ↔ `eval.run` scorecards snapshot linkage
