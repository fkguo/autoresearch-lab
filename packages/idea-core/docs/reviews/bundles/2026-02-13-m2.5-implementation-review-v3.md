# Review Bundle: 2026-02-13 M2.5 Implementation v3 (post-v2 Opus blockers)

## 1) Goal / Scope / Out-of-Scope

Goal:
- Complete M2.5 only: minimal observable/reproducible Multi-Island state machine with strict contract compliance.

In scope (cumulative this stage):
- `search.step` implemented with deterministic state transitions and audit artifact.
- Observable path includes `STAGNANT` and `REPOPULATED`.
- `campaign.status` and `search.step` expose consistent `island_states`.
- Budget and step interactions covered (local fuse, global exhaustion, exact-boundary behavior).
- `best_score` improvement path makes `CONVERGING` reachable and resets stagnation.
- Idempotency replay for `search.step` prevents double-spend.
- Explicitly document M2.5 semantics:
  - island `EXHAUSTED` is terminal in this phase;
  - non-step token/cost/time budget accounting is deferred to M2.6.

Out of scope (unchanged):
- M2.6 operator execution and non-empty `new_node_ids` generation.
- Cross-repo contract edits in `idea-generator`.

## 2) Diff Evidence

### 2.1 Commits under review

- `532ed32` `feat(m2.5): add minimal multi-island search.step state machine`
- `38efc55` `fix(m2.5): add best-score driven converging transition`
- `8968c19` `test(m2.5): document exhausted terminal and budget edge semantics`

### 2.2 `git show --stat`

```text
532ed32 feat(m2.5): add minimal multi-island search.step state machine
 src/idea_core/engine/service.py                    | 242 ++++++++++++++++++++-
 .../test_search_step_island_state_machine.py       | 168 ++++++++++++++
 tests/engine/test_service_rank_and_idempotency.py  |   9 +-
 3 files changed, 413 insertions(+), 6 deletions(-)
38efc55 fix(m2.5): add best-score driven converging transition
 src/idea_core/engine/service.py                    | 68 +++++++++++++++++++---
 .../test_search_step_island_state_machine.py       | 63 ++++++++++++++++++++
 2 files changed, 123 insertions(+), 8 deletions(-)
8968c19 test(m2.5): document exhausted terminal and budget edge semantics
 src/idea_core/engine/service.py                    |  7 ++++
 .../test_search_step_island_state_machine.py       | 43 ++++++++++++++++++++++
...
rg -n "def campaign_complete|def search_step|def node_get|def node_list|def node_promote" src/idea_core/engine/service.py
```
...
`rg` single-definition sanity check

```text
595:    def campaign_complete(self, params: dict[str, Any]) -> dict[str, Any]:
721:    def search_step(self, params: dict[str, Any]) -> dict[str, Any]:
888:    def node_get(self, params: dict[str, Any]) -> dict[str, Any]:
905:    def node_list(self, params: dict[str, Any]) -> dict[str, Any]:
1411:    def node_promote(self, params: dict[str, Any]) -> dict[str, Any]:
```

### 3.3 Failures and fixes in this round

- Addressed prior `best_score`/`CONVERGING` blocker by implementing score-improvement transition logic and tests.
- Addressed prior edge-semantics blockers by:
  - adding explicit `EXHAUSTED` terminal branch and comment;
  - adding regression for `n_steps == max_steps` (campaign ends exhausted without `early_stopped` flag);
  - adding regression that `step_budget.max_tokens` is a documented no-op in M2.5.
- No validation/test regressions after fixes.

## 4) Contract Alignment Checklist (MUST)

- [x] `search.step` result conforms to `search_step_result_v1.schema.json`.
- [x] `campaign.status` result conforms to `campaign_status_v1.schema.json`.
- [x] `island_states` entries conform to `island_state_v1.schema.json`.
- [x] Required M2.5 observable transition path exists and is tested:
  - `SEEDING -> EXPLORING -> STAGNANT -> REPOPULATED`.
- [x] `search.step` and `campaign.status` return consistent `island_states` after mutation.
- [x] Budget interaction coverage includes:
  - local `step_budget.max_steps` fuse (`step_budget_exhausted`),
  - global `max_steps` exhaustion (`budget_exhausted`),
  - exact boundary `n_steps == max_steps` semantics,
  - idempotent replay without additional `steps_used`.
- [x] `make validate` remains green; no OpenRPC/schema drift.
...
- `contracts/idea-generator-snapshot/schemas/island_state_v1.schema.json`
- `contracts/idea-generator-snapshot/schemas/campaign_status_v1.schema.json`
- `contracts/idea-generator-snapshot/schemas/search_step_result_v1.schema.json`
- `contracts/idea-generator-snapshot/schemas/idea_core_rpc_v1.openrpc.json` (campaign.status + search.step excerpt)
        ...
        *,
        ...
        if method == "search.step":
            ...
            if campaign is None or campaign.get("last_step_id") != step_id:
                return False
            return self._search_step_artifact_path(campaign_id, step_id).exists()
        return False
    ...
    def search_step(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.store.mutation_lock(campaign_id):
            ...
            local_usage: dict[str, float] = {
                # M2.5 only charges step count here. Token/cost/time accounting is
                # deferred until M2.6 when search.step executes real operators.
                "tokens": 0.0,
                "cost_usd": 0.0,
                "wall_clock_s": 0.0,
                "steps": 0.0,
...
def test_search_step_reaches_stagnant_then_repopulated_and_status_matches(tmp_path: Path) -> None:
    ...
    assert island_after_stagnant["population_size"] == 2
    ...
    assert island_after_repopulate["population_size"] == 2
...
def test_search_step_step_budget_max_tokens_is_noop_in_m2_5(tmp_path: Path) -> None:
    ...
    # M2.5 does not consume tokens during search.step, so token fuse is a no-op.
    assert result["n_steps_executed"] == 2
    assert "early_stopped" not in result
...
### FILE: contracts/idea-generator-snapshot/schemas/idea_core_rpc_v1.openrpc.json (campaign.status + search.step excerpt)
