# Review Bundle: 2026-02-13 M1.0+M1.6 + M2(min) Implementation v2

## 1) Goal / Scope / Non-Goals

### Goal
Implement executable `idea-core` baseline with contract-first validation and auditable idempotency replay.

### Scope
- Independent repo bootstrap for `idea-core`.
- Contract source strategy: read-only vendored snapshot + explicit sync.
- M1 tooling (`validate`, `$ref` closure, drift-guard, commandized workflow).
- M2 minimal RPC loop:
  - `campaign.init/status`
  - `eval.run`
  - `rank.compute`
  - `node.promote`
  - idempotency replay/conflict
- Required regression tests listed by SSOT task.

### Non-Goals
- No `search.step` implementation in this phase.
- No arXiv/INSPIRE ingestion in this phase.
- No optimization of ranking strategy quality (stub-deterministic baseline only).

---

## 2) Diff Stat + Key Diffs
...
### Diff stats
```text
352a8f4: 54 files changed, 4548 insertions(+)
824b009:  9 files changed, 1715 insertions(+)
```
...
352a8f4 feat(m1): bootstrap repo and contract validation toolchain
 ...
 Makefile                                           |  21 +
 README.md                                          |  26 +
 .../idea-generator-snapshot/CONTRACT_SOURCE.json   |   8 +
 .../abstract_problem_registry_v1.schema.json       |  49 ++
 .../schemas/budget_envelope_v1.schema.json         |  38 ++
 .../schemas/budget_limit_v1.schema.json            |  16 +
 .../schemas/budget_snapshot_v1.schema.json         |  46 ++
 .../schemas/budget_topup_v1.schema.json            |  16 +
 .../schemas/campaign_charter_v1.schema.json        |  91 +++
 .../schemas/campaign_init_result_v1.schema.json    |  48 ++
 .../campaign_mutation_result_v1.schema.json        | 143 ++++
 .../schemas/campaign_status_v1.schema.json         |  79 +++
 .../schemas/distributor_event_v1.schema.json       |  57 ++
 .../distributor_policy_config_v1.schema.json       | 161 +++++
 .../distributor_state_snapshot_v1.schema.json      |  37 ++
 .../schemas/elo_config_v1.schema.json              |  33 +
 .../schemas/eval_result_v1.schema.json             |  35 +
 .../schemas/evaluator_config_v1.schema.json        |  24 +
 .../schemas/failed_approach_v1.schema.json         |  21 +
 .../schemas/formalism_registry_v1.schema.json      |  31 +
 .../schemas/idea_campaign_v1.schema.json           |  75 +++
 .../schemas/idea_card_v1.schema.json               | 100 +++
 .../schemas/idea_core_rpc_v1.openrpc.json          | 718 +++++++++++++++++++++
 .../schemas/idea_evidence_graph_v1.schema.json     |  45 ++
 .../schemas/idea_handoff_c2_v1.schema.json         | 151 +++++
 .../schemas/idea_list_filter_v1.schema.json        |  18 +
 .../schemas/idea_node_v1.schema.json               | 286 ++++++++
 .../schemas/idea_novelty_report_v1.schema.json     |  33 +
 .../schemas/idea_scorecards_v1.schema.json         | 183 ++++++
 .../schemas/idea_seed_pack_v1.schema.json          |  16 +
 .../schemas/idea_selection_v1.schema.json          |  31 +
 .../schemas/idea_tournament_v1.schema.json         |  55 ++
 .../schemas/idempotency_meta_v1.schema.json        |  21 +
 .../schemas/island_state_v1.schema.json            |  22 +
 .../schemas/node_list_result_v1.schema.json        |  22 +
 .../schemas/node_mutation_log_v1.schema.json       |  37 ++
 .../schemas/novelty_delta_table_v1.schema.json     |  45 ++
 .../schemas/promotion_result_v1.schema.json        | 206 ++++++
 .../schemas/ranking_result_v1.schema.json          |  50 ++
 .../schemas/rationale_draft_v1.schema.json         |  30 +
 .../schemas/reduction_audit_v1.schema.json         | 345 ++++++++++
 .../schemas/reduction_report_v1.schema.json        | 252 ++++++++
 .../schemas/rpc_error_data_v1.schema.json          |  73 +++
 .../schemas/search_step_result_v1.schema.json      | 171 +++++
 .../schemas/seed_pack_v1.schema.json               |  30 +
 pyproject.toml                                     |  28 +
 scripts/sync_contracts_snapshot.sh                 |  38 ++
 src/idea_core/__init__.py                          |   3 +
 src/idea_core/contracts/__init__.py                |   1 +
 src/idea_core/contracts/catalog.py                 | 119 ++++
 src/idea_core/contracts/validate.py                | 251 +++++++
 tests/conftest.py                                  |   9 +
 tests/contracts/test_validate_contracts.py         |  66 ++
 54 files changed, 4548 insertions(+)

824b009 feat(m2): implement minimal rpc loop with idempotency and ranking gates
 src/idea_core/engine/__init__.py                   |    1 +
 src/idea_core/engine/reduction.py                  |   60 ++
 src/idea_core/engine/service.py                    | 1079 ++++++++++++++++++++
 src/idea_core/engine/store.py                      |  114 +++
 src/idea_core/engine/utils.py                      |   23 +
 src/idea_core/rpc/__init__.py                      |    1 +
 src/idea_core/rpc/server.py                        |  114 +++
 .../engine/test_reduction_and_schema_contracts.py  |   91 ++
 tests/engine/test_service_rank_and_idempotency.py  |  232 +++++
 9 files changed, 1715 insertions(+)
```

---

## 3) Commands + Outputs + Failure/Fix Log
...
### Iteration log (failed -> fixed)
1. drift-guard false positive on scalar-array OpenRPC param (`eval.run.node_ids`) -> adjusted rule to allow scalar arrays only.
2. Elo result could violate `ranking_result` minItems when usable nodes < 2 -> moved insufficient_nodes check to usable scored set.

---

## 4) Contract Alignment Checklist

- M1 tooling implemented and tested.
- Implemented methods: `campaign.init`, `campaign.status`, `eval.run`, `rank.compute`, `node.promote`.
- `rank.compute`:
  - MUST failure order path covered by test
  - ignores `status=failed` scorecards for observed dimensions
  - returns `effective_dimensions` + `scorecards_artifact_ref`
- `node.promote`:
  - grounding gate
  - reduction gate (`reduction_audit_missing` / `reduction_audit_not_pass`)
- Idempotency:
  - replay first response
  - `idempotency_key_conflict` includes `idempotency_key` + `payload_hash`
- Schema-level invariants tested:
  - reduction_audit partial logic
  - distributor_event `rng_seed_used => rng_alg`

---

## 5) Full Key Files (for audit)
...
class IdeaCoreService:
    ...
    @staticmethod
    def _hash_without_idempotency(params: dict[str, Any]) -> str:
        filtered = {k: v for k, v in params.items() if k != "idempotency_key"}
    ...
    def campaign_init(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        p_hash = self._hash_without_idempotency(params)
        ...
        formalism_registry = params.get("formalism_registry") or {
            "entries": [
                {
        ...
        abstract_problem_registry = params.get("abstract_problem_registry") or {
            "entries": [
                {
    ...
    def eval_run(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        scorecards_ref = self.store.write_artifact(
            campaign_id,
            "scorecards",
            artifact_name,
            scorecards_payload,
        ...
        campaign["last_scorecards_artifact_ref"] = scorecards_ref
        campaign["usage"]["steps_used"] += 1
        self._set_campaign_running_if_budget_available(campaign)
    ...
    def rank_compute(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        ranking_artifact_ref = self.store.write_artifact(
            campaign_id,
            "rankings",
            f"ranking-{uuid4()}.json",
            ranking_artifact,
        )
    ...
    def node_promote(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        handoff_ref = self.store.write_artifact(
            campaign_id,
            "handoff",
            f"handoff-{node_id}.json",
            handoff_payload,
        )
