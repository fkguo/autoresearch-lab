# Review Bundle: 2026-02-13 M2.10 Implementation v7 (post-v6 blocker fixes)

## 1) Goal / Scope / Out-of-Scope

Goal:
- Close remaining v6 blockers from dual review for M2.10 gate.

In scope (this delta):
- `eval.run` MUST reject cross-campaign nodes with `-32014 node_not_in_campaign`.
- Read methods use lock for portable consistency (`campaign.status`, `node.get`, `node.list`).
- Defensive campaign payload consistency check in `_load_campaign_or_error`.
- Unimplemented declared methods return validated `method_not_implemented` error data.
- Packaging explicitly declares `referencing` dependency.
- Add regression tests for the above.

Out of scope (unchanged in this delta):
- arXiv/literature ingestion.
- search policy implementation beyond current stub contract behavior.
- ranking algorithm optimization beyond M2 contract correctness.

## 2) Diff Evidence

### 2.1 Commit under review

- `cf40549` `fix(m2): close v6 blockers for method stubs and eval membership`

### 2.2 `git show --stat`

```text
cf40549 fix(m2): close v6 blockers for method stubs and eval membership
 .../schemas/idea_core_rpc_v1.openrpc.json          |   3 +
 pyproject.toml                                     |   1 +
 src/idea_core/engine/service.py                    | 135 ++++++++++++---------
 tests/engine/test_service_rank_and_idempotency.py  |  49 ++++++++
 4 files changed, 134 insertions(+), 54 deletions(-)
```

### 2.3 Key diffs (auditable excerpts)
...
+
+def test_stub_method_returns_method_not_implemented_error(tmp_path: Path) -> None:
+    ...
+    assert exc.code == -32000
+    assert exc.data["reason"] == "method_not_implemented"
+    assert exc.data["details"]["method"] == "search.step"
```
...
## 3) Commands Run + Outputs + Fix Log
...
### 3.2 Output (verbatim key lines)

```text
python3 -m pip install -e .[dev]
...
Requirement already satisfied: referencing<1,>=0.35.0 ...
...
...
### 3.3 Failures and fixes in this round

- No red test in this round after applying fixes.
- Fixes applied directly against v6 blockers (B1/B2/B3/B4/B5).

## 4) Contract Alignment Checklist (MUST)

- [x] `eval.run` exposes `scorecards_artifact_ref` and idempotency replay semantics.
- [x] `rank.compute` failure priority order is covered:
  - `no_scorecards` before `insufficient_dimensions` before `insufficient_nodes`.
- [x] `rank.compute` ignores failed scorecards for observed dimensions.
- [x] `node.promote` gating enforced:
  - grounding pass required;
  - reduction gate (`missing`/`not_pass`) and registry check enforced.
- [x] Idempotency conflict includes `idempotency_key` + `payload_hash` in error data.
- [x] `reduction_audit` partial logic covered (`mixed satisfied+pending`, `toy_check_result=skipped`, optional `auditor_origin`).
- [x] `distributor_event` schema rule covered (`rng_seed_used` => `rng_alg` required).
- [x] Declared-but-not-yet-implemented methods return schema-valid `method_not_implemented` error data.
- [x] `$ref` closure / drift-guard / OpenRPC minimal checks remain green under `make validate`.

## 5) Files Included for Review Packet

- `Makefile`
- `pyproject.toml`
- `src/idea_core/contracts/validate.py`
- `src/idea_core/contracts/catalog.py`
- `src/idea_core/engine/store.py`
- `src/idea_core/engine/reduction.py`
- `src/idea_core/engine/service.py`
- `tests/engine/test_service_rank_and_idempotency.py`
- `tests/engine/test_node_promote.py`
- `tests/engine/test_node_read_methods.py`
- `tests/engine/test_reduction_and_schema_contracts.py`
- `contracts/idea-generator-snapshot/schemas/idea_core_rpc_v1.openrpc.json`
- `contracts/idea-generator-snapshot/schemas/rpc_error_data_v1.schema.json`

## 6) Full File Snapshots (for clean-room audit)
    ...
    try:
        service.handle(
            "search.step",
            {
                "campaign_id": campaign_id,
                "n_steps": 1,
                "idempotency_key": "search-step-stub",
            },
        )
        assert False, "expected RpcError"
    except RpcError as exc:
