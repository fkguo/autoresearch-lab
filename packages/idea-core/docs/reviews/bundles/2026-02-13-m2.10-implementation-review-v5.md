# Review Bundle: 2026-02-13 M1+M2(min) Implementation v5 (full contract packet)

## 1) Goal / Scope / Non-Goals

### Goal
Complete M1 contract toolchain + M2 minimal executable loop with strict contract alignment, replay-safe idempotency, and auditable outputs.

### Scope
Included commits:
- `352a8f4` M1 bootstrap and contract validation toolchain
- `824b009` M2 minimal runtime
- `ae260a5` hardening fixes (JCS/default hash/locking/atomicity/promotion tests)
- `0193371` node discovery read APIs (`node.get`, `node.list`)
- `d3eadd7` portable lock abstraction + prepared/committed idempotency staging

### Non-Goals
- `search.step` and additional campaign mutation methods are not implemented in this phase.
- Ingestion pipelines (arXiv/INSPIRE) are out of scope for this phase.
...
- Dual-review metadata target: reviewer_b backend must be `gemini` with `fallback_reason=null`.
...
- Idempotency:
  - JCS hash
  - defaults filled before hash
  - conflict error includes `idempotency_key` + `payload_hash`
  - staged record (`prepared` -> `committed`) for crash tolerance
- Promotion gates and ranking failure-order covered by tests.
...
## 5) Full Source + Full Contract Files
