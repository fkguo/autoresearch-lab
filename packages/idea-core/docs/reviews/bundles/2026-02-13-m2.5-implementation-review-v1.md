# Review Bundle: 2026-02-13 M2.5 Implementation v1

## 1) Goal / Scope / Out-of-Scope

Goal:
- Implement M2.5 (Multi-Island minimal state machine) with observable/reproducible island transitions, without extending into M2.6 operator execution.

In scope (this delta):
- Implement `search.step` with deterministic minimal island state transitions and budget/step interaction.
- Ensure `search.step` and `campaign.status` surface consistent `island_states` snapshots.
- Add M2.5 regression tests for transition triggers, post-transition field consistency, and budget/step fuse behavior.
- Preserve contract compliance (`make validate` + schema validation at runtime) and idempotency replay semantics.

Out of scope (explicitly unchanged in this delta):
- M2.6 operator interface and dummy operator-generated new nodes.
- Any expansion beyond minimal transition path requirements for M2.5.
- Changes to `idea-generator` schemas/OpenRPC SSOT.
...
```diff
commit 532ed32e72ff8fd053949efa1aa513d9480e1523
Author: FK Guo <11114855+fkguo@user.noreply.gitee.com>
Date:   Fri Feb 13 14:10:08 2026 +0800

    feat(m2.5): add minimal multi-island search.step state machine

diff --git a/src/idea_core/engine/service.py b/src/idea_core/engine/service.py
index f793595..ee9cadb 100644
--- a/src/idea_core/engine/service.py
+++ b/src/idea_core/engine/service.py
@@ -14,6 +14,7 @@ from idea_core.engine.utils import payload_hash, sha256_hex, utc_now_iso
...
+STAGNATION_PATIENCE_STEPS = 2
 ...
 @dataclass
@@ -271,6 +272,16 @@ class IdeaCoreService:
...
@@ -583,8 +594,237 @@ class IdeaCoreService:
...
+                from_state, to_state, reason = self._advance_island_state_one_tick(island)
+                transition_events.append(
+                    {
+                        "tick": tick + 1,
+                        "island_id": island.get("island_id", f"island-{chosen_index}"),
...
diff --git a/tests/engine/test_search_step_island_state_machine.py b/tests/engine/test_search_step_island_state_machine.py
new file mode 100644
index 0000000..a76929c
--- /dev/null
+++ b/tests/engine/test_search_step_island_state_machine.py
@@ -0,0 +1,168 @@
...
- No red tests after landing M2.5 implementation and regression suite.

## 4) Contract Alignment Checklist (MUST)

- [x] `search.step` now returns schema-valid `SearchStepResult` with required fields:
  - `campaign_id`, `step_id`, `n_steps_requested`, `n_steps_executed`, `new_node_ids`, `updated_node_ids`, `island_states`, `budget_snapshot`, `idempotency`.
- [x] `campaign.status` and `search.step` both source `island_states` from campaign manifest, yielding consistent snapshots across calls.
- [x] Minimal observable transition path is reproducible:
  - `SEEDING -> EXPLORING -> STAGNANT -> REPOPULATED`.
- [x] Budget/step interaction is bounded and test-covered:
  - `step_budget.max_steps` local fuse triggers `early_stopped=true` with `early_stop_reason=step_budget_exhausted`.
  - global `max_steps` exhaustion triggers `early_stop_reason=budget_exhausted` and campaign status `exhausted` with no step overrun.
- [x] Idempotency replay for `search.step` does not double spend steps (`steps_used` stable under replay).
- [x] No contract drift introduced; `make validate` remains green.
...
- `contracts/idea-generator-snapshot/schemas/idea_core_rpc_v1.openrpc.json` (method excerpts)
