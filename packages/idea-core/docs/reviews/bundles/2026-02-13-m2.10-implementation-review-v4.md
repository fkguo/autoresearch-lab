# Review Bundle: 2026-02-13 M1+M2(min) Implementation v4 (post-v3 fixes)

## 1) Goal / Scope / Non-Goals

### Goal
Deliver contract-aligned, test-backed minimal executable `idea-core` baseline with replay-safe idempotency and promotion gates.

### Scope
Included commits:
- `352a8f4` M1 bootstrap + contract tooling
- `824b009` M2 minimal runtime
- `ae260a5` blocker fixes (JCS/locking/atomicity/registry merge/promotion tests)
- `0193371` read APIs (`node.get`, `node.list`) to close discovery gap

### Non-Goals
- `search.step` and campaign mutation methods beyond `init/status` remain out of scope for this stage.
- arXiv/INSPIRE ingestion out of scope.
...
### Commits
```text
0193371 feat(m2): add node.get/node.list discovery read APIs
...
```
- Added `node.get` and `node.list` to handler dispatch.
- Added read-path tests (`tests/engine/test_node_read_methods.py`).
...
- M1 tooling: implemented and green.
- Implemented RPC methods now include:
  - `campaign.init`
  - `campaign.status`
  - `node.get`
  - `node.list`
  ...
  - `node.promote`
- Idempotency:
  - JCS hash + default filling before hash
  - replay + conflict path with `idempotency_key` and `payload_hash`
- `rank.compute` MUST semantics:
  - failure order tested
  - failed scorecards ignored for observed keys
- `node.promote` gates:
  - grounding gate
  - formalism gate
  - reduction gate paths all tested
- distributor/reduction schema invariants tested.
