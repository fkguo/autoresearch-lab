# Review Bundle: 2026-02-13 M3.2 Operators >=3 family (v1)
...
2. Different islands produce different node “styles”
   - Added `operator_selection_policy="island_index_v1"` for `hep.operators.v1` so each `island-N` deterministically maps to operator `N % 3`.
3. Repopulation does not collapse diversity
   - Multi-island state machine already triggers `STAGNANT -> REPOPULATED`; selection remains island-indexed so families stay distinct across repopulation ticks.
4. Multi-island campaign seeding for operator execution
   ...
   - Seeds are distributed round-robin across islands; fail-fast if `initial_island_count > seed_count` (prevents no-parent islands that would never execute operators).
...
- `docs/reviews/bundles/2026-02-13-m3.2-validate-and-pytest-v1.txt`
- Result: `make validate` OK + `56 passed`
...
index c791919..0d17ac7 100644
--- a/src/idea_core/engine/operators.py
+++ b/src/idea_core/engine/operators.py
@@ -145,3 +145,146 @@ def default_search_operators() -> tuple[SearchOperator, ...]:
...
+                "backend_id": self.backend_id,
+            },
+            evidence_uris_used=["https://example.org/hep/operator/symmetry"],
+        )
+
+
...
+                "backend_id": self.backend_id,
+            },
+            evidence_uris_used=["https://example.org/hep/operator/limit-explorer"],
+        )
+
+
...
@@ -371,6 +372,52 @@ class IdeaCoreService:
             return resolved
         return []
...
+
     def _resolve_domain_pack_for_charter(
         self,
         charter: dict[str, Any],
@@ -621,6 +668,7 @@ class IdeaCoreService:
         seed: dict[str, Any],
         formalism_id: str,
         index: int,
+        island_id: str,
         ...
         node_id = str(uuid4())
@@ -651,7 +699,7 @@ class IdeaCoreService:
             "node_id": node_id,
             "revision": 1,
             "parent_node_ids": [],
-            "island_id": "island-0",
+            "island_id": island_id,
             ...
             "origin": {
@@ -696,6 +744,19 @@ class IdeaCoreService:
             domain_pack, enabled_pack_ids = self._resolve_domain_pack_for_charter(params["charter"])
...
@@ -756,16 +817,7 @@ class IdeaCoreService:
                     "steps_used": 0,
                     "nodes_used": 0,
                 },
-                "island_states": [
...
+                "island_states": self._initial_island_states(initial_island_count),
                 ...
                 "domain_pack": {
@@ -779,12 +831,29 @@ class IdeaCoreService:
                 formalism_registry,
...
+                    island_id=island_id,
                 ...
                 try:
@@ -810,7 +879,7 @@ class IdeaCoreService:
...
+            self._refresh_island_population_sizes(campaign, nodes)
             ...
             result = {
                 "campaign_id": campaign_id,
@@ -929,6 +998,21 @@ class IdeaCoreService:
...
+        return IdeaCoreService._next_search_operator(runtime, search_operators)
+
     def _build_operator_node(
         self,
         *,
@@ -1208,7 +1292,12 @@ class IdeaCoreService:
