# Review Bundle: 2026-02-13 M2.5 Implementation v2 (post-v1 Gemini blockers)

## 1) Goal / Scope / Out-of-Scope

Goal:
- Deliver M2.5 only: a minimal observable/reproducible Multi-Island state machine with `search.step` + `campaign.status` island state consistency.

In scope (this delta, cumulative):
- `search.step` implementation with deterministic island transitions.
- Reproducible `STAGNANT -> REPOPULATED` path and `EXHAUSTED` handling.
- `step_budget` + global budget interaction without overrun.
- Idempotency prepared/committed replay safety for `search.step`.
- Add `best_score`-improvement hook so `CONVERGING` is reachable and stagnation can reset.
- Regression tests for transition triggers, field consistency, budget interactions, and replay non-double-spend.

Out of scope (unchanged):
- M2.6 operator interface and operator-produced new nodes (`new_node_ids` intentionally remains empty in this stage).
- Any schema/OpenRPC edits in `idea-generator` SSOT.
...
`rg` single-definition sanity check (addresses v1 duplicate-definition concern)

```text
595:    def campaign_complete(self, params: dict[str, Any]) -> dict[str, Any]:
716:    def search_step(self, params: dict[str, Any]) -> dict[str, Any]:
881:    def node_get(self, params: dict[str, Any]) -> dict[str, Any]:
898:    def node_list(self, params: dict[str, Any]) -> dict[str, Any]:
1404:    def node_promote(self, params: dict[str, Any]) -> dict[str, Any]:
```

### 3.3 Failures and fixes in this round

- v1 Gemini blocker `best_score` path addressed by adding score-improvement detection and `CONVERGING` transition reset.
- Added regression tests:
  - single-tick `SEEDING -> EXPLORING`
  - best-score improvement drives `EXPLORING/CONVERGING` path into `CONVERGING` with counter reset.
- No test or validation failures after fixes.

## 4) Contract Alignment Checklist (MUST)

- [x] `campaign.status` returns `campaign_status_v1`-conformant payload including `island_states`.
- [x] `search.step` returns `search_step_result_v1`-conformant payload (required fields present; no extra fields).
- [x] `island_states` includes schema-required fields and state values from `island_state_v1` enum.
- [x] M2.5 observable transition path exists and is test-covered:
  - `SEEDING -> EXPLORING -> STAGNANT -> REPOPULATED`.
- [x] `search.step` and `campaign.status` expose consistent `island_states` snapshots after each mutation.
- [x] Budget interaction correctness is test-covered:
  - local `step_budget` fuse stops early with `early_stop_reason=step_budget_exhausted`.
  - global `max_steps` exhaustion stops with `early_stop_reason=budget_exhausted`, campaign `status=exhausted`, no overspend.
- [x] `search.step` idempotency replay returns same result with `idempotency.is_replay=true`, without extra `steps_used` consumption.
- [x] `make validate` green; no contract drift introduced.
