...
> - `scripts/lib/front-door-boundary-authority.mjs`、`scripts/check-shell-boundary-anti-drift.mjs` 与 `packages/hep-mcp/tests/docs/docToolDrift.test.ts` 现会 fail-closed 这些 first-touch docs 的 generic-first ordering，而不只检查片段存在；formal review / self-review 也已在当前 worktree 上收敛为 `0` blocking。closeout 同轮吸收了 `Gemini(auto)` 提出的 `docs/PROJECT_STATUS.md` workflow list reordering amendment，并继续把 README / QUICKSTART / PROJECT_STATUS 的 order anchors 收紧到 guard 中，避免这类 prominence drift 再次静默回流
>
> **v1.9.25 Changelog**:
