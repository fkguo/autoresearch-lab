...
- [ ] 仍缺剩余 unrepointed Pipeline A command set 的 bounded repoint/delete closeout slice（installable public shell 现主要剩 residual non-computation `run` workflows 与相邻 workflow/support commands；`doctor` / `bridge` / `literature-gap` 已不再通过 `hepar` / `hep-autoresearch` public alias 暴露，但 internal full parser 仍保留给 maintainer/eval/test paths；`autoresearch run --workflow-id computation` 已是 live TS slice）
...
- **2026-04-07 follow-up（public-shell narrowing）**: installable `hepar` / `hep-autoresearch` / `hep-autopilot` public shell 现进一步移除了 `doctor`、`bridge` 与 `literature-gap`，所以这些命令都不再是 operator-facing installable alias 的 live surface；它们当前只保留在 `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` internal full parser 里供 maintainer/eval/test paths 使用。当前剩余的 public Pipeline A shell 因此进一步收窄到 residual non-computation `run` workflows 与相邻 workflow/support commands；这仍不构成 delete/rehome closeout，本轮不声称这些 internal full-parser compatibility paths 已整体删除。
