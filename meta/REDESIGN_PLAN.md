...
- follow-up planning lane now audits the remaining internal full-parser residues (`doctor`, `bridge`, `literature-gap`, `paper_reviser`, `method-design`, `run-card`, `branch`) via `meta/docs/prompts/prompt-2026-04-08-legacy-residue-audit.md` before the next deletion slice, keeping the delete-vs-keep decisions source-grounded
> - immediate next seams now narrow further: surviving internal full-parser residues (`doctor`, `bridge`, `literature-gap`, `paper_reviser`, `method-design`, `run-card`, `branch`) still need delete-vs-keep rebaseline, while the first structural runtime batch remains `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`; the checked-in runtime deep dive plus delegated lane packet now keep `DelegatedRuntimeHandleV1` as the first post-cleanup implementation slice
>
> **v1.9.43 Changelog**:
