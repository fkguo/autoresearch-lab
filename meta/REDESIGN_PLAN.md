...
> **版本**: 1.9.29-draft (v1.9.28 + CP-OBJ-01D landed)
> **日期**: 2026-04-07
> **基线**: v1.9.27-draft
...
>
> **2026-04-07 next-control-plane planning note**: after current runtime hardening, post-runtime eval ratification, package-doc/front-door rebaseline, and the now-formally-closed root first-touch ordering cleanup, the next generic-first main axis remains `CP-OBJ-01 — Orchestrator Control-Plane Object Convergence`. `CP-OBJ-01A` now exists as the source-grounded object-map / authority-spec bundle (`meta/docs/prompts/prompt-2026-04-07-cp-obj-01a-object-map-authority-spec.md` + `meta/docs/2026-04-07-orchestrator-control-plane-object-map.md`), `CP-OBJ-01B` is landed as the typed delegated execution identity seam, `CP-OBJ-01C` is landed as the source-recorded delegated runtime projection seam, and `CP-OBJ-01D` is now landed via `meta/docs/prompts/prompt-2026-04-07-cp-obj-01d-unified-operator-read-model.md` plus the shared internal helper `packages/orchestrator/src/operator-read-model-summary.ts`: runtime diagnostics summary, team scoping/view task projection, assignment approval attention, and root run ledger-status interpretation now consume one bounded operator vocabulary without widening public host/team payloads or promoting `runtime_projection` into new authority. The next intended code slice is therefore `CP-OBJ-01E` via `meta/docs/prompts/prompt-2026-04-07-cp-obj-01e-research-task-bridge-into-live-execution.md`: add the smallest typed bridge that lets canonical `ResearchTask` authority survive through live execution without rewriting team runtime into `ResearchLoopRuntime`. Fresh parallel census also narrows `M-22` remainder into a later two-step cleanup rather than one blurry tail: first Python legacy root approval/run authority, then `research_workflow_v1` / workflow-template `WorkflowGateSpec` residue. The separate docs lane `DOCS-FRONTDOOR-01` remains formally closed via `meta/docs/prompts/prompt-2026-04-07-docs-frontdoor-01-root-first-touch-ordering.md`, `README.md`, `docs/README_zh.md`, `docs/PROJECT_STATUS.md`, `docs/QUICKSTART.md`, the upgraded order-aware front-door anti-drift checks, and zero-blocking formal review/self-review.
