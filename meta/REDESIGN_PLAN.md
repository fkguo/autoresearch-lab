...
> - front-door truth now matches that deletion across live docs / fixtures / tests: `docs/TESTING_GUIDE.md`, `meta/protocols/session_protocol_v1.md`, package README/tutorial/workflow/orchestrator docs, `meta/front_door_authority_map_v1.json`, `scripts/lib/front-door-authority-map.mjs`, and `scripts/lib/front-door-boundary-authority.mjs` all describe `literature-gap` as a deleted parser shell rather than a surviving internal compatibility path
> - the residual Pipeline A support-surface truth is therefore narrower and more explicit: `doctor` / `bridge` / `literature-gap` are all deleted parser shells, live `internal_support_commands` is now empty, installable public `hepar` still exposes only `run` as a compatibility pointer with no public workflow ids, and the remaining delete-first queue now starts at `method-design`, then `run-card`, then `branch`, while `paper_reviser` still waits for a real replacement lane
> - checked-in next-lane planning is now aligned with that post-delete truth instead of the earlier audit framing: `meta/docs/prompts/prompt-2026-04-08-pipeline-a-residual-support-surfaces-sequenced-lanes.md` is the active residual support-surface packet, `meta/docs/prompts/prompt-2026-04-08-legacy-residue-audit.md` is retained only as a superseded redirect anchor, and the front-door blind-spot guardrail bundle (`meta/docs/prompts/prompt-2026-04-08-front-door-drift-blind-spot-locks.md` + `meta/docs/plans/2026-04-08-front-door-drift-blind-spot-lane-plan.md`) is queued as a sidecar follow-up rather than claimed as already landed truth
> - targeted acceptance is green on the current worktree: `git diff --check`; `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m pytest -q packages/hep-autoresearch/tests/test_literature_gap_runner.py packages/hep-autoresearch/tests/test_public_cli_surface.py`; `pnpm --filter @autoresearch/literature-workflows test -- tests/resolve.test.ts`; `pnpm --filter @autoresearch/orchestrator test -- tests/autoresearch-cli.test.ts`; `node scripts/check-shell-boundary-anti-drift.mjs`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`
> - formal review + self-review now also converge with `0` blocking on the final worktree: `Opus`/Claude canonical review = `CONVERGED`, `OpenCode(zhipuai-coding-plan/glm-5.1)` canonical workspace review = `CONVERGED`, and `Gemini(auto)` = `CONVERGED` via same-model embedded-source rerun after the canonical review-mode path failed in `run_gemini.sh` because proxy/no_proxy export input was polluted by terminal-control sequences. The only directly related amendment was adopted: `packages/hep-autoresearch/workflows/C1_literature_gap.md` now includes `packages/hep-autoresearch/tests/test_public_cli_surface.py` in the executable-proof block so the maintainer fixture matches the stated acceptance evidence
...
> - next closure queue at that point stayed bounded and delete-first: `literature-gap` still needed consumer proof moved off the parser before honest deletion, while `method-design` / `run-card` / `branch` remained later cleanup slices and `paper_reviser` still waited for a real replacement lane
>
> **v1.9.47 Changelog**:
...
> - current worktree lands the next delete-first Pipeline A contraction instead of letting internal parser residue linger as fake authority: parser-level wrappers `approvals`, `report`, `logs`, `context`, `smoke-test`, `propose`, `skill-propose`, and `migrate` are removed entirely from `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py`, while only `method-design`, `run-card`, and `branch` remain in that support-command bucket
> - the same slice re-homes verification to the real lower-level owners instead of preserving wrapper coupling: `packages/hep-autoresearch/tests/test_approval_packet.py` now validates approval-packet trio authority without `cmd_approvals_show`, `packages/hep-autoresearch/tests/test_mcp_doctor_and_bridge_cli.py` no longer treats deleted `smoke-test` as live truth, and package/docs/authority-map surfaces now consistently describe these wrappers as deleted parser residue whose durable authority lives in lower-level toolkit/test paths; the final amendment pass also removes stale `start` / `checkpoint` / `request-approval` / `reject` claims from current internal full-parser truth because those verbs are no longer registered commands
> - scoped acceptance passed on this slice: `git diff --check`; `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m pytest -q packages/hep-autoresearch/tests/test_public_cli_surface.py`; `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m pytest -q packages/hep-autoresearch/tests/test_approval_packet.py packages/hep-autoresearch/tests/test_report_renderer.py packages/hep-autoresearch/tests/test_evolution_proposal.py packages/hep-autoresearch/tests/test_evolution_trigger.py packages/hep-autoresearch/tests/test_migrate.py packages/hep-autoresearch/tests/test_mcp_doctor_and_bridge_cli.py`; `node scripts/check-shell-boundary-anti-drift.mjs`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`
> - GitNexus post-change evidence was refreshed on the dirty worktree with `npx gitnexus analyze --force`; `detect_changes(scope=all)` reports `changed_files=16`, `risk_level=low`, and no affected processes, which is consistent with a bounded parser/docs/test shrink rather than new generic authority widening
> - formal review + self-review now also converge with `0` blocking on this slice: `Opus` canonical review = `CONVERGED`, `Gemini(auto)` = `CONVERGED` via same-model compact embedded-source rerun after the earlier canonical proxy-env bootstrap failure, and `OpenCode(zhipuai-coding-plan/glm-5.1)` = `CONVERGED` via same-model compact embedded-source rerun after the earlier workspace pass stayed discovery-only. All directly related amendments were adopted: stronger `doctor` / `bridge` exact-substring assertions, the public-shell drift-gate comment, explicit `skill-propose` deletion wording in package docs, matching `propose` / `skill-propose` wording in `README.zh.md`, and the front-door authority correction that narrows `internal_support_commands` to `doctor` / `bridge` / `literature-gap`
- the implementation prompt for the completed delete-first lane is now checked in at `meta/docs/prompts/prompt-2026-04-07-pipeline-a-delete-first-internal-support-wrappers.md`, and the next structural lane packet is checked in at `meta/docs/prompts/prompt-2026-04-07-delegated-runtime-handle-v1.md`
- follow-up planning lane first seeded the remaining internal full-parser residue audit via `meta/docs/prompts/prompt-2026-04-08-legacy-residue-audit.md`; that file is now superseded by `meta/docs/prompts/prompt-2026-04-08-pipeline-a-residual-support-surfaces-sequenced-lanes.md`, which reflects the later post-delete truth and exact execution order
> - immediate next seams at that point still narrowed toward surviving internal full-parser residues (`doctor`, `bridge`, `literature-gap`, `paper_reviser`, `method-design`, `run-card`, `branch`) plus the first structural runtime batch `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`; the later sequenced-lane packet now further advances that queue beyond `doctor` / `bridge` / `literature-gap`
>
> **v1.9.43 Changelog**:
> - latest completed upstream `main` CI truth remains GitHub Actions `CI` run `24075936865` (2026-04-07) on head `5949270` (`refactor: retire idea-map and tighten idea-engine discovery surface`), while later same-day main pushes are still newer than the last completed green run
> - current worktree closes the next concrete `idea-engine` authority leaks instead of leaving them as “later asset cleanup”: checked-in `idea-engine` discovery cards now point to TS-owned OpenRPC under `packages/idea-engine/contracts/...`, runtime default contract/builtin HEP pack assets now live under package-local `packages/idea-engine/contracts/**` + `packages/idea-engine/assets/hep_builtin_domain_packs.json`, and `idea-mcp` now requires an explicit `IDEA_MCP_DATA_DIR` outside the dev repo rather than silently falling back to repo-local `packages/idea-engine/runs`
> - scoped acceptance passed on this slice: `git diff --check`; `pnpm --filter @autoresearch/idea-engine build`; `pnpm --filter @autoresearch/idea-engine test`; `pnpm --filter @autoresearch/idea-mcp test -- tests/server.test.ts tests/rpc-client.test.ts tests/rpc-client.integration.test.ts tests/tool-registry.test.ts`; `pnpm --filter @autoresearch/idea-mcp build`; `pnpm --filter @autoresearch/rep-sdk test -- tests/discovery-agent-card.test.ts tests/discovery-agent-registry.test.ts`; `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m pytest packages/hep-autoresearch/tests/test_agent_registry.py -q`
> - GitNexus post-change evidence was refreshed on the dirty worktree with `npx gitnexus analyze --force`; `detect_changes(scope=all)` stays `MEDIUM` only because generated appendix/review surfaces are also modified, while the live blast radius remains bounded (`impact(resolveIdeaDataDir, upstream)=LOW`; `impact(IdeaRpcClient, upstream)=HIGH` only through `startServer` plus direct tests)
> - formal review + self-review converged with `0` blocking on this slice: `Opus = CONVERGED`, `Gemini(auto) = CONVERGED` (same-model embedded-source rerun under a non-login shell after the first two runner attempts died in shell/env bootstrap noise), and `OpenCode(zhipuai-coding-plan/glm-5.1) = CONVERGED`; the only directly related low-risk amendment was adopted in the same batch by strengthening `packages/idea-mcp/tests/server.test.ts` with an absolute repo-local rejection case, while the proposed extra positive-path `IdeaRpcClient` test was declined/closed because the existing `campaign.init` / `campaign.status` / `search.step` / `eval.run` tests already exercise valid explicit `rootDir` flows
> - immediate next seams therefore narrow again: internal-only Pipeline A delete-first cleanup, then the deeper runtime structural sequence `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`; the remaining `idea-core` work is broader parity / retire-all closure, not lingering default host/asset authority drift
>
> **v1.9.42 Changelog**:
> - latest known upstream `main` CI truth is now GitHub Actions `CI` run `24075936865` (2026-04-07) on head `5949270` (`refactor: retire idea-map and tighten idea-engine discovery surface`), so the current batch continues from a green mainline rather than a live blocker
> - current worktree lands the next bounded Pipeline A front-door contraction instead of leaving public support residue half-alive: installable `hep-autoresearch` / `hepar` / `hep-autopilot` public shell now exposes exactly one command, `run`, and that `run` remains only a compatibility pointer with no public workflow ids
> - the same slice retires the remaining public support verbs from the installable shell without reopening generic authority: `approvals`, `report`, `logs`, `context`, `smoke-test`, `propose`, `skill-propose`, and `migrate` have now been deleted from the parser entirely, while `method-design`, `run-card`, and `branch` remain internal full-parser-only maintainer residue; `meta/front_door_authority_map_v1.json`, `scripts/lib/front-door-authority-map.mjs`, public CLI tests, and the shared boundary wording fixture now all lock that narrower post-delete truth across first-touch package docs plus `EVOLUTION.zh.md`
> - current targeted acceptance reran successfully on this slice: `git diff --check`; `python3 -m pytest packages/hep-autoresearch/tests/test_public_cli_surface.py -q`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`; `node scripts/check-shell-boundary-anti-drift.mjs`
> - formal review + self-review also converged with `0` blocking on this slice: counted reviewer results are `Opus = CONVERGED`, `Gemini(auto) = CONVERGED` (same-model embedded-source rerun after the known canonical proxy-env bootstrap failure), and `OpenCode(zhipuai-coding-plan/glm-5.1) = CONVERGED` via same-model embedded-source normalization; the earlier OpenCode workspace discovery pass remains supplemental discovery evidence rather than the counted gate artifact. Review health telemetry for this bounded closeout is `{review_rounds: 2, first_round_blocking: false, final_zero_blocking: true, reviewer_disagreement: false, amendments_total: 0, amendments_adopted: 0, amendments_deferred: 0, amendments_declined_closed: 0, packet_assumption_breach: false, self_review_caught_new_issue: false, reopened_later: false, post_closeout_escape: false, front_door_audit_performed: true}`
> - immediate next seams therefore shift again from “retire/rehome residual public support-surface commands” to the remaining internal-only cleanup queue: contract rebaseline and delete-vs-keep decisions for surviving internal full-parser residues (`doctor`, `bridge`, `literature-gap`, `paper_reviser`, `method-design`, `run-card`, and `branch`), then the now-concrete `idea-engine` authority cleanup (retire the lingering `idea-core` OpenRPC contract pointer in the `idea-engine` agent card and fail-close `idea-mcp` away from the repo-local default `idea-engine/runs` data root), and only then descend into the deeper runtime structural batch `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`
>
> **v1.9.41 Changelog**:
> - latest known upstream `main` CI truth is now GitHub Actions `CI` run `24073265627` (2026-04-07) on head `9e1af5b` (`docs: add next seam implementation prompts`), so the current batch continues from a green mainline while closing already-landed local slices
> - current worktree closes three bounded generic-first seams instead of reopening architecture scope: installable public `hepar run` now keeps the shell verb but exposes no public workflow ids; `paper_reviser` joins `ingest` / `reproduce` / `revision` / `literature_survey_polish` / `shell_adapter_smoke` as internal full-parser coverage only; `TeamExecutionState.pending_approvals` is removed as persisted authority in favor of assignment-local approval metadata plus a derived `live_status.pending_approvals`; and `orch_fleet_enqueue` no longer treats `readRunListView()` as a mutation gate, using canonical `state.json` / `ledger.jsonl` / `artifacts/runs/<runId>` evidence instead
> - the same slice tightens front-door truth rather than leaving a compatibility survivor half-alive: `meta/front_door_authority_map_v1.json`, `scripts/lib/front-door-authority-map.mjs`, package docs, and public CLI tests now all agree that installable `hepar run` is only a compatibility pointer to canonical `autoresearch run --workflow-id ...`, while `paper_reviser` coverage remains internal full-parser only
> - current targeted acceptance reran successfully on this slice: `git diff --check`; `pnpm --filter @autoresearch/orchestrator test -- tests/orch-fleet-queue.test.ts`; `pnpm --filter @autoresearch/orchestrator test -- tests/team-unified-runtime.test.ts tests/team-unified-runtime-sequential.test.ts tests/team-unified-runtime-parallel-recovery.test.ts tests/team-execution-state.test.ts`; `pnpm --filter @autoresearch/orchestrator test -- tests/team-execution-runtime.test.ts tests/orchestrator.test.ts`; `pnpm --filter @autoresearch/hep-mcp test -- tests/contracts/orchRunExecuteAgent.team.test.ts tests/contracts/orchRunExecuteAgent.team-view.test.ts`; `python3 -m pytest packages/hep-autoresearch/tests/test_public_cli_surface.py -q`; `python3 -m pytest packages/hep-autoresearch/tests/test_paper_reviser_workflow.py -q`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`; `node scripts/check-shell-boundary-anti-drift.mjs`; `pnpm --filter @autoresearch/orchestrator build`; `pnpm --filter @autoresearch/hep-mcp build`
> - immediate next seams therefore shifted from “clear the last public run workflow” to the then-open closure queue; after the later same-day delete-first follow-up removed `smoke-test` / `migrate` / `approvals` / `report` / `logs` / `context` / `propose` / `skill-propose`, the surviving residue is now the narrower internal full-parser set `doctor` / `bridge` / `literature-gap` / `paper_reviser` / `method-design` / `run-card` / `branch`, followed by the later `idea-engine` asset/contract authority cleanup and then the deeper runtime structural batch `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`
>
> **v1.9.40 Changelog**:
...
> - current worktree lands the first concrete next-batch generic closure slice instead of leaving it as planning-only: installable public `hepar run --workflow-id ...` is now narrowed to `paper_reviser` only, while `ingest`, `reproduce`, `revision`, `literature_survey_polish`, and `shell_adapter_smoke` are explicitly classified as internal full-parser coverage for maintainer/eval/regression use
> - the same slice also lands the first typed front-door authority-map fixture: `meta/front_door_authority_map_v1.json` plus `scripts/lib/front-door-authority-map.mjs` now classify `autoresearch_cli`, `hepar_public_shell`, `hepar_internal_full_parser`, and `orchestrator_mcp_tools_spec` as distinct live surfaces consumed by docs/tests rather than implicit wording alone
> - package-level first-touch docs are now aligned with that exact survivor truth: `packages/hep-autoresearch/docs/BEGINNER_TUTORIAL*.md` and `docs/ORCHESTRATOR_INTERACTION*.md` no longer teach retired public `ingest` / `reproduce` / `revision` paths, and public `paper_reviser` E2E coverage now goes through the installable public wrapper instead of the internal full parser entrypoint
> - current targeted acceptance reran successfully on this slice: `git diff --check`, `python3 -m pytest packages/hep-autoresearch/tests/test_public_cli_surface.py -q`, `python3 -m pytest packages/hep-autoresearch/tests/test_paper_reviser_workflow.py -q`, `python3 -m pytest packages/hep-autoresearch/tests/test_adapter_gate_resolution_cli.py -q`, `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`, `pnpm --filter @autoresearch/orchestrator test -- tests/autoresearch-cli.test.ts`, `node scripts/check-shell-boundary-anti-drift.mjs`
> - immediate next seams are therefore no longer “public run residue first cut” but `idea-engine` default-host authority first cut, projection-only operator/read-model guard, and the remaining residual Pipeline A support-surface classification / retirement work
>
> **v1.9.35 Changelog**:
...
> - 该 closeout 继续保持 bounded：`research_task_ref` 只作为 internal typed bridge seam 存在于 team-local relation storage / lineage sync 中，`TeamAssignmentSession.task_status` / `task_lifecycle_status` 仍只是 projection；`team` tool schema、`live_status`、`replay`、`assignment_results` 都没有新增 public fields，也没有把 team runtime 重写成 `ResearchLoopRuntime`
> - `meta/.review/2026-04-07-cp-obj-01e-r1/` 的 formal trio review + self-review 已在当前 worktree 收敛为 `0` blocking：counted reviewer results 为 `Opus = CONVERGED`、`Gemini(auto) = CONVERGED`、`OpenCode(zhipuai-coding-plan/glm-5.1) = CONVERGED`；canonical `Opus` packet 因 source mismatch 产出了不可归档 verdict，`Gemini(auto)` canonical review-mode 仍因 `run_gemini.sh` proxy-env bootstrap 失败而无产出，`OpenCode` canonical workspace pass 则保留为 discovery 信号并通过 same-model embedded rerun 做 formal gate normalization。当前已吸收的直接相关低风险 amendments 是：`DelegatedFollowupTeamExecutionMetadata.research_task_ref` 改用显式 `ResearchTaskExecutionRef` 类型、删除 `buildTeamConfigForDelegatedFollowupTask(...)` 中的 dead null branch、以及移除 `primeDelegatedFollowupTeamState(...)` 未使用的 `permissions` 参数；self-review 同轮还修复了 `packages/orchestrator/package.json` 的 clean-build hygiene gap（`build = tsc --build`，`clean` 同步删除 `tsconfig.tsbuildinfo`）
> - 当前收口方式与外部成熟实现保持一致：内部 canonical lineage 留在持久 session/state substrate，operator/read-model 继续走单独 projection 层，而不是把 canonical references 直接推到 public summary surface；这与 `codex-rs/app-server-protocol/src/protocol/thread_history.rs` 对 canonical turn id 的保留方式，以及 `claude-code-sourcemap/restored-src/src/utils/sessionStorage.ts` 把 transcript/session lineage 与 UI-only progress 分层的做法一致
> - `CP-OBJ-01` 的 object-convergence 主轴现完成到 `01E`；下一批 generic-first mainline closure 应转回 `M-22A`（Python legacy root approval/run authority residue）与 `M-22B`（`research_workflow_v1` / `WorkflowGateSpec` workflow-template residue）两条后续清理线，而不是重新打开 public payload redesign
>
> **v1.9.29 Changelog**:
> - `CP-OBJ-01D` 已在当前 worktree landed：`packages/orchestrator/src/operator-read-model-summary.ts` 现集中表达 shared operator vocabulary / summary helper，统一 runtime diagnostics summary、assignment approval attention、assignment -> task lifecycle/status projection、以及 ledger event -> root run status mapping；`runtime-diagnostics-bridge.ts`、`team-execution-scoping.ts`、`team-execution-view.ts`、`orch-tools/run-read-model.ts` 现复用同一解释层，而不是继续并列维护多套 operator interpreter
> - 该 closeout 继续保持 bounded：root run authority 仍 rooted in `RunState` + ledger，team/runtime public payload 未扩大，没有把 `runtime_projection`、transcript、`job`、`thread` 或 task bridge 提升成新 authority；本轮吸收的低风险 amendment 只包括 `VisibleRunStatusFilter` 与实际可产出状态对齐、对应 filter regression assertions，以及 `status_*` passthrough 的意图注释
> - `meta/.review/2026-04-07-cp-obj-01d-r1/` 的 formal trio review + self-review 已在当前 worktree 收敛为 `0` blocking：`Opus = CONVERGED`，`Gemini(auto) = CONVERGED`（canonical review-mode path 仍因 `run_gemini.sh` shell/proxy env 污染失败，已按仓规完成 same-model embedded-source rerun + 显式 proxy env），`OpenCode(zhipuai-coding-plan/glm-5.1) = CONVERGED_WITH_AMENDMENTS`；关于 `awaiting_approval` / `needs_recovery` session `ended_at` 的建议经源码复核后已 declined/closed，因为现有 `TeamAssignmentSession` 语义是 execution-attempt lineage，approval resume 本就应开启 child session
> - `CP-OBJ-01` 的 next-code truth 现推进为 `CP-OBJ-01E`；并行 fresh census 也把 `M-22` 剩余面进一步拆清为：先做 Python legacy root approval/run authority (`M-22A`)，再做 `research_workflow_v1` / `WorkflowGateSpec` residue cleanup (`M-22B`)
>
> **v1.9.28 Changelog**:
> - 新增 checked-in implementation prompt `meta/docs/prompts/prompt-2026-04-07-cp-obj-01d-unified-operator-read-model.md`，把 `CP-OBJ-01D` 明确收束为：在不扩大 public host/team payload 的前提下，统一 root run read model、team live status、runtime diagnostics 的 operator vocabulary / summary helper，而不是再造一个大而全的新 view object
> - 新增 checked-in follow-on prompt `meta/docs/prompts/prompt-2026-04-07-cp-obj-01e-research-task-bridge-into-live-execution.md`，把 `CP-OBJ-01E` 明确收束为：为 canonical `ResearchTask` 补一条进入 live execution 的 typed bridge seam，而不是把 team runtime 重写成 `ResearchLoopRuntime`
> - `CP-OBJ-01` 的 next checked-in code truth 现明确锁到 `CP-OBJ-01D`；并行审计也确认 `M-22` 剩余 authority residue 不应与 `01D` 混做，下一步更适合作为 `01D -> 01E -> M-22 remainder` 的顺序推进
>
> **v1.9.27 Changelog**:
...
> - 新增 checked-in implementation prompt `meta/docs/prompts/prompt-2026-04-07-cp-obj-01c-delegated-runtime-session-turn-projection.md`，把 `CP-OBJ-01C` 明确收束为：在 source path 上记录 compact delegated runtime session/turn projection、让 `executeDelegatedAgentRuntime` 与 `runtime-diagnostics-bridge` 消费同一 projection seam、并把最小稳定 projection sideband 挂到真实 `TeamAssignmentSession` 上，而不是扩大成 transcript/job/public API redesign
> - 新增 checked-in implementation prompt `meta/docs/prompts/prompt-2026-04-07-docs-frontdoor-01-root-first-touch-ordering.md`
> - 同一批 current-worktree docs follow-up 已把 `README.md`、`docs/README_zh.md`、`docs/PROJECT_STATUS.md`、`docs/QUICKSTART.md` 的 first-touch ordering/prominence 收口为 generic-first：`autoresearch` lifecycle / workflow-plan / bounded computation 现先于 `@autoresearch/hep-mcp` domain-pack example 出现，root README / 中文 README 也先给 generic lifecycle/control-plane smoke path，再给 HEP smoke path；`docs/QUICKSTART.md` 则在保持 domain-pack quickstart 身份的前提下，先给 generic front-door first-touch block，再进入 HEP path
> - `scripts/lib/front-door-boundary-authority.mjs`、`scripts/check-shell-boundary-anti-drift.mjs` 与 `packages/hep-mcp/tests/docs/docToolDrift.test.ts` 现会 fail-closed 这些 first-touch docs 的 generic-first ordering，而不只检查片段存在；formal review / self-review 也已在当前 worktree 上收敛为 `0` blocking，并吸收了两条低风险 guard amendment（QUICKSTART 首个 HEP 步骤锚点、README 章节过渡锚点）
>
> **v1.9.25 Changelog**:
...
> - 新增 checked-in prompt `meta/docs/prompts/prompt-2026-04-07-package-legacy-first-touch-docs-rebaseline.md`，把 `packages/hep-autoresearch/` 这条 package-level first-touch docs 链路收口到 generic-first truth：repo-root `README.md` / `docs/QUICKSTART.md` / `docs/TESTING_GUIDE.md` 仍是默认 front door，而 package docs 降为 package-local legacy / maintainer docs
> - 同一批 current-worktree follow-up 也把 package-level README / index / beginner tutorial / workflow / interaction wording 接入 shared front-door fixture 与 doc drift guard，使 installable legacy shell 不再在 package docs 中被写成 public computation / `doctor` / `bridge` / `literature-gap` 仍然存活的默认入口
> - post-runtime eval bundle 现以 tracker-only umbrella `POST-RT-EVAL-01` 进入 machine-readable truth：这 ratify 了 three bounded slices 的存在与边界，但不新增 phase-counted remediation item，也不改变 aggregate counts
>
> **v1.9.22 Changelog**:
...
> - 明确当前 remaining duplicate-authority families：`packages/orchestrator` 首个 TS slice 已不再手维护 A1–A5 runtime authority；`skills/research-team/scripts/gates/convergence_schema.py` 的 convergence metadata adoption 已于 2026-04-02 landed (`d7daa1e`)；仍待后续 broader rollout 的 authority family 现收敛为 `packages/hep-autoresearch/src/hep_autoresearch/toolkit/{orchestrator_state.py,computation.py}` 的 Python legacy approval authority、以及 `meta/schemas/research_workflow_v1.schema.json` 的独立 `WorkflowGateSpec` / workflow templates
...
>
> **v1.9.14 Changelog**:
> - source-grounded rebaseline 关闭 `NEW-02` / `NEW-03` / `NEW-04`：`packages/hep-autoresearch/src/hep_autoresearch/toolkit/{approval_packet,report_renderer}.py`、`orchestrator_cli.py` 与相邻 tests 早已提供 live approval trio / report-renderer authority；旧 template/codegen-specific 文案降格为历史实现意图而非 today truth。随后 2026-04-07 delete-first lane 已删除 parser-level `approvals show` / `report render` wrappers
> - 明确 approval cluster authority boundary：`packages/orchestrator/src/computation/approval.ts` 已有 bounded A3 trio producer，`packages/orchestrator/src/orch-tools/{run-read-model,approval}.ts` 已消费 `approval_packet_v1.json`，当前 truth 不再依赖 Python parser wrapper surfaces
...
- **缺失的显式 closeout 计划 (2026-03-24 planning completeness correction)**: 当前 repo 仍缺两个单独 bounded retirement slices，不能只停留在“Phase 4+ 退役”的高层口径：
  - `Pipeline A run-surface repoint / parity / delete`: 收口当前剩余的 Pipeline A Python delegation（installable public shell 现已进一步收窄：public `run --workflow-id computation` 已从 `hepar` / `hep-autoresearch` 退到 native TS `autoresearch` front door，`doctor` / `bridge` / `literature-gap` 与旧 public support commands 也都已从 installable shell 退到 internal full parser，installable public inventory 只剩 bare `run` compatibility pointer）；一旦 TS surface 通过验收，直接删除 `hep-autoresearch` / `hepar`
...
- [ ] 仍缺剩余 unrepointed Pipeline A command set 的 bounded repoint/delete closeout slice（installable public shell 现主要剩 residual non-computation `run` workflows 与相邻 workflow/support commands；public `run --workflow-id computation` 已不再通过 `hepar` / `hep-autoresearch` alias 暴露，`doctor` / `bridge` / `literature-gap` 也已退到 internal full parser 给 maintainer/eval/test paths；`autoresearch run --workflow-id computation` 已是 live TS slice）
...
>
> **Consumer rollout narrowing (2026-03-29, TS approvals first)**: first implementation scope was explicitly bounded to `packages/shared` + `packages/orchestrator` approval authority adoption only. The current worktree now lands that bounded TS approval/query/state-validation/read-model slice: `packages/orchestrator` live-reads shared GateSpec authority for A1–A5, keeps `A0` compatibility-only outside `GateSpec v1`, and defers Python legacy approval mappings plus `research_workflow_v1` / template cleanup. (The `research-team` convergence gate consumer adoption later landed on 2026-04-02; see next bullet.) The broader `M-22` consumer rollout therefore remains pending even though the first real non-test consumer slices are now live.
>
...
>
> **2026-04-07 remainder closeout (M-22A first cut + M-22B residue removal)**: the deferred remainder is no longer “future tense” in aggregate. `packages/hep-autoresearch` no longer owns generic root lifecycle mutations for `init/pause/resume/approve/export`; those commands now thin-pass through canonical `autoresearch`, the installable public legacy shell excludes direct root-mutation verbs `start` / `checkpoint` / `request-approval` / `reject`, keeps only `run` on the public shell, and `web/app.py` is now read-only diagnostics. Separately, `research_workflow_v1` / `workflow-templates` and their generated/test residue have been removed from live schema/codegen/test surfaces, leaving recipe-based workflow authority as the only surviving live path. The next delete-first follow-up then removed parser-level wrappers `approvals` / `report` / `logs` / `context` / `smoke-test` / `propose` / `skill-propose` / `migrate` entirely, so `M-22` now remains in progress only for the narrower surviving provider-local residue rebaseline (`doctor` / `bridge` / `literature-gap` / `paper_reviser` / `method-design` / `run-card` / `branch`) plus adjacent package-local legacy docs, not for any remaining installable public support surface, generic lifecycle, or workflow authority itself.
...
- **2026-04-07 follow-up（public-shell narrowing, phase 3 + authority-map first cut）**: `meta/docs/prompts/prompt-2026-04-07-next-batch-public-run-residue-retirement.md` 与 `prompt-2026-04-07-next-batch-front-door-authority-map.md` 的首刀最初把 installable public `hepar run --workflow-id ...` 收窄到只剩 `paper_reviser`，并将 `ingest` / `reproduce` / `revision` / `literature_survey_polish` / `shell_adapter_smoke` 明确降为 internal full-parser coverage；同时新增 `meta/front_door_authority_map_v1.json` + `scripts/lib/front-door-authority-map.mjs`，把 `autoresearch_cli`、`hepar_public_shell`、`hepar_internal_full_parser` 与 `orchestrator_mcp_tools_spec` 明确分类成独立 live surfaces，并由 `packages/orchestrator/tests/autoresearch-cli.test.ts`、`packages/hep-mcp/tests/docs/docToolDrift.test.ts`、`packages/hep-autoresearch/tests/test_public_cli_surface.py` 共同 fail-close。package-level tutorial / interaction docs 也同步停止把 retired public `ingest` / `reproduce` / `revision` 路径教回 front door。
- **2026-04-07 follow-up（public-shell narrowing, phase 4）**: 当前 worktree 继续完成这条收窄线而不是保留最后一个 compatibility survivor：installable public `hepar run` 现在不再暴露任何 workflow ids，`paper_reviser` 也退回 internal full-parser coverage，public help 文案收敛为指向 canonical `autoresearch run --workflow-id ...` 的 compatibility pointer。相应地，`meta/front_door_authority_map_v1.json`、`scripts/lib/front-door-authority-map.mjs`、`packages/hep-autoresearch/tests/test_public_cli_surface.py`、`packages/hep-autoresearch/tests/test_paper_reviser_workflow.py`、以及 package-level README/tutorial/workflow/orchestrator docs 现都以“public workflow ids 为空，`paper_reviser` 仅 internal”作为当前真相。
- **2026-04-07 same-day health/authority completion（internal `paper_reviser` error-path finalization + authority-map parity guard）**: the immediately following bounded fix keeps this slice truthful instead of leaving fresh regressions behind: `cmd_run()` now treats `allow_errors` as an internal-only optional flag so internal full-parser `paper_reviser` failures fail closed instead of crashing with `AttributeError`, `packages/hep-autoresearch/tests/test_paper_reviser_workflow.py` now covers that internal structured-error path, and the front-door authority fixture continues to restore `export` in the internal lifecycle-adapter group while `packages/hep-mcp/tests/docs/docToolDrift.test.ts` asserts JS-helper/JSON parity for surface ids, owners, source anchors, and internal command groups.
...
> **Rebaseline (2026-03-29, source audit; 2026-04-07 delete-first update)**: `M-20` is already live as a baseline-landed migration-infrastructure item. Commit `85f816f` added `meta/schemas/migration_registry_v1.schema.json`, checked-in `meta/schemas/migration_registry_v1.json`, `packages/hep-autoresearch/src/hep_autoresearch/toolkit/migrate.py`, historical parser wiring, and `packages/hep-autoresearch/tests/test_migrate.py`; the later 2026-04-07 delete-first cleanup removed the parser-level `migrate` wrapper while leaving the lower-level toolkit/test authority intact.
>
...
| `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` | historical parser wrapper removed; no parser-level `migrate` command remains |
...
> **Rebaseline (2026-03-29, source audit; 2026-04-07 delete-first update)**: the historical `approvals show --run-id <RID> --gate <A?> --format short|full|json` wrapper has now been removed from both installable and internal parser surfaces. Live authority remains the trio artifact producer/renderer path (`toolkit/approval_packet.py` + `_request_approval()` write path + adjacent tests), while TS orchestrator currently offers read/list consumers (`run-read-model.ts`, `approval.ts`) rather than a direct parser-level `show` replacement.
...
| 文件 | 修改内容 |
|---|---|
| `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` | historical parser wrapper removed; no parser-level `approvals show` command remains |
| `packages/hep-autoresearch/tests/test_approval_packet.py` | trio render/write + gate-context regressions on toolkit authority; no parser-wrapper coupling |
...
**验收检查点**:
- [x] parser-level `approvals` wrapper removed from both installable and internal parser surfaces
- [x] trio artifacts remain generated as `packet_short.md` / `packet.md` / `approval_packet_v1.json` under `artifacts/runs/<run_id>/approvals/<approval_id>/`
- [x] lower-level approval packet authority remains test-covered without parser-wrapper coupling

### NEW-04: 自包含人类报告生成 ✅ Rebaselined closeout

> **Rebaseline (2026-03-29, source audit; 2026-04-07 delete-first update)**: the first live self-contained report substrate already exists. `packages/hep-autoresearch/src/hep_autoresearch/toolkit/report_renderer.py` renders Markdown/LaTeX directly and `packages/hep-autoresearch/tests/test_report_renderer.py` covers run summary, key numbers, audit pointers, SHA256 completeness, and TeX output. The parser-level `report render` wrapper has now been removed.
>
> **Authority boundary**: this surface remains Python toolkit authority today; parser-level CLI wrappers are removed, and TS orchestrator still does not provide a generic `report render` replacement.
...
| `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` | historical parser wrapper removed; no parser-level `report render` command remains |
...
**验收检查点**:
- [x] parser-level `report` wrapper removed from both installable and internal parser surfaces
...
- [x] `toolkit/report_renderer.py` 可生成可编译的 LaTeX 文件
...
| internal artifacts / canonical lifecycle | `packet_short.md` 作为默认快速审阅载体写入审批目录；批准动作本身走 canonical `autoresearch approve` |
...
## 操作
- 批准: `autoresearch approve A3-0001`
- 拒绝: internal full-parser `reject A3-0001 --reason "..."`
- 完整 packet: 打开 `artifacts/runs/<run_id>/approvals/<approval_id>/packet.md`
```
...
- [x] packet_short 作为默认快速审阅载体写入审批目录
- [x] packet_short ~60 行软上限，超限时附加 `overflow: artifacts/runs/<run_id>/approvals/<approval_id>/packet.md` 指针
...
> **Historical rebaseline (2026-04-07)**: `NEW-IDEA-01` 仍保持 `done`，但 live truth 已进一步从“TS default host first cut”收紧为“installable public path 只有 TS host”。当前 current-worktree follow-up 之后，`idea-mcp` 的 active public host path 只剩 in-process TS `idea-engine`；旧的 Python `idea-core` host path、`IDEA_MCP_BACKEND` 选择器与 `IDEA_CORE_PATH` knob 已从 installable public surface 删除并改为 fail-closed。public MCP tool inventory 也继续维持 exact public surface：`campaign.init`、`campaign.status`、`search.step`、`eval.run`。A later same-day authority cleanup then closed the remaining default contract/data-root leaks as well: checked-in `idea-engine` discovery cards now point to `packages/idea-engine/contracts/...`, runtime default contract and builtin HEP pack assets now live under package-local TS-owned paths, and `idea-mcp` requires explicit `IDEA_MCP_DATA_DIR` outside the dev repo instead of defaulting to repo-local `packages/idea-engine/runs`. This still does not claim `idea-core retire-all`; the remaining follow-up is broader parity / retire-all closeout rather than restoring any Python-side host or default asset fallback.
...
- [ ] lower-level `approval_packet` / `report_renderer` authority 可用（不依赖 parser-level wrappers）
...
>
> **Current post-runtime eval bundle truth (ratified 2026-04-07)**:
...
>
> On the current `main`, these files are no longer just future entrypoints: the corresponding bounded implementations are already present via `68a790a`, `3da2d2e`, and `a6cc685`, with review evidence under `meta/.review/2026-04-06-post-runtime-*`.
>
> Governance scope is now explicit:
> - `68a790a` ratifies a typed multi-axis eval outcome contract in the existing `packages/hep-mcp/src/eval/*` substrate, not a new orchestrator-local eval stack
> - `3da2d2e` ratifies an orchestrator-owned runtime diagnostics bridge artifact that summarizes `run-manifest`, `spans.jsonl`, runtime markers, and terminal state, without promoting that bridge into a second runtime authority
...
>
> **2026-04-07 control-plane convergence update**: after current runtime hardening, post-runtime eval ratification, package-doc/front-door rebaseline, and the now-formally-closed root first-touch ordering cleanup, `CP-OBJ-01 — Orchestrator Control-Plane Object Convergence` is now landed through `CP-OBJ-01E`. `CP-OBJ-01A` remains the checked-in object-map / authority-spec bundle, `CP-OBJ-01B` the typed delegated execution identity seam, `CP-OBJ-01C` the source-recorded delegated runtime projection seam, `CP-OBJ-01D` the unified operator read-model interpreter, and `CP-OBJ-01E` the bounded research-task bridge into live execution. The landed `01E` seam is deliberately internal-only: `packages/orchestrator/src/computation/feedback-followups.ts` now derives a canonical `research_task_ref` from `ResearchTask` + delegated handoff, `primeDelegatedFollowupTeamState(...)` primes an internal sidecar task-ref registry before `orch_run_execute_agent` launches, and `packages/orchestrator/src/team-unified-runtime.ts` synchronizes that same canonical ref across assignment/session/checkpoint lineage without widening public `team` payload, `live_status`, `replay`, or `assignment_results`. This keeps canonical task authority in research-loop while making live execution state finally retain a typed relation to it. With that bounded convergence slice landed, the next intended generic-first cleanup now shifts to `M-22A` (Python legacy root approval/run authority residue) followed by `M-22B` (`research_workflow_v1` / workflow-template `WorkflowGateSpec` residue). The separate docs lane `DOCS-FRONTDOOR-01` remains formally closed via `meta/docs/prompts/prompt-2026-04-07-docs-frontdoor-01-root-first-touch-ordering.md`, `README.md`, `docs/README_zh.md`, `docs/PROJECT_STATUS.md`, `docs/QUICKSTART.md`, the upgraded order-aware front-door anti-drift checks, and zero-blocking formal review/self-review.
...
**2026-04-03 bounded first-slice closeout**: `NEW-RT-10` 的首个 bounded slice 已 landed，并将 team-local delegated runtime 从单 `pending_approval` slot 推进到 agent/assignment/session-aware state：当前代码现实已具备 agent-scoped session lineage、delegated approval ownership on assignment metadata plus root approval sentinel read-model、`ResearchTask`-projected background-task lifecycle、以及 fail-closed delegated approval intervention。较早把这层 truth 表述成“persisted pending-approval collection”的 wording 已被 2026-04-07 的 projection-guard follow-up supersede；当前 authority 不在第二套持久 approval 集合里，而在 assignment 自身 canonical metadata 上。formal trio review 在 `Gemini` 原轮 + `OpenCode` same-model embedded rerun + `Opus` 原轮上收敛为 `0` blocking。该 closeout 仍不等于 full done：更宽的 sub-agent context inheritance、MCP inheritance / additive override、以及 cleanup / termination semantics 仍属于本 item 后续 slice。
...
2. **C2 — Delegated approval authority boundary**
   - delegated approval truth 应留在 assignment-local canonical metadata（`approval_id`、`approval_packet_path`、`approval_requested_at`、`delegate_id`）上，而不是再引入 `pending_approvals: AgentPendingApproval[]` 这类第二套 persisted collection / typed map。
   - `live_status.pending_approvals` 可以保留，但必须始终是从 assignment/session 关系现算得到的 projection；root run approval 继续走现有 run-level sentinel / read-model path，避免 delegated runtime 与 root run 继续争抢同一 authority，同时也避免为了分离而再造第二个 durable registry。
...
**`NEW-RT-10` 验收口径**:
- [ ] delegated runtime 的 session / approval state 不再与 root run 共享单槽位 truth，也不再依赖第二套 persisted approval projection
...
>
> **Follow-up update (2026-04-06)**: 当前 worktree 进一步把 canonical launcher-backed literature front door 收口到 native TS `autoresearch workflow-plan`：该命令现在面向已初始化外部 project root，直接通过 `@autoresearch/literature-workflows` 解析 recipe，并持久化 `.autoresearch/state.json#/plan` 与派生 `.autoresearch/plan.md`；checked-in 的 `python3 skills/research-team/scripts/bin/literature_fetch.py workflow-plan` 则被明确降为同一 authority 的 lower-level parallel consumer。对应 front-door docs、`scripts/check-shell-boundary-anti-drift.mjs`、`packages/hep-mcp/tests/docs/docToolDrift.test.ts` 已同步改写，不再把 Python shell 误写成 delegation target，并且显式写明 `autoresearch init` 前置条件与 stateful persistence side effects。实现侧同轮补上 `plan_id` parity，并把 `packages/orchestrator/tests/autoresearch-cli.test.ts` 扩展到 init fail-closed、active `run_status` guard、fallback `run_id` 三条直接行为门禁。Follow-up acceptance passed: `git diff --check`; `pnpm --filter @autoresearch/orchestrator test -- tests/autoresearch-cli.test.ts`; `pnpm --filter @autoresearch/orchestrator build`; `node scripts/check-shell-boundary-anti-drift.mjs`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`. Formal follow-up review converged after R2 surfaced real blockers on doc authority drift and missing run-status coverage: `Opus` R3 = `CONVERGED`, `Gemini-3.1-Pro-Preview` R3 = `CONVERGED`, and `OpenCode(zhipuai-coding-plan/glm-5.1)` required a same-model verdict-normalization rerun after exploratory non-JSON R3 output, ending at `CONVERGED`. Formal self-review then rechecked code/docs/tests/authority alignment and found 0 blocking.
...
> **Closeout update (2026-03-26, amended 2026-04-06, CI/package-doc/run-surface follow-ups 2026-04-07)**: NEW-SHELL-01 is now implemented as the planned bounded guardrail slice, without expanding into runtime or package redesign. The live enforcement surfaces are: root checker `scripts/check-shell-boundary-anti-drift.mjs`; shared boundary test `packages/shared/src/__tests__/package-boundary-authority.test.ts`; orchestrator boundary test `packages/orchestrator/tests/package-boundary.test.ts`; the extended host-consumption contract `packages/hep-mcp/tests/contracts/sharedOrchestratorPackageExports.test.ts`; and, after the 2026-04-06 follow-up, root/front-door wording locks in `packages/hep-mcp/tests/docs/docToolDrift.test.ts`. The 2026-04-07 CI follow-up then moved the root checker from closeout-only acceptance into always-on mainline enforcement by wiring `node scripts/check-shell-boundary-anti-drift.mjs` into the Node 22 `build-and-test` leg in `.github/workflows/ci.yml` before `pnpm -r build`, so front-door wording drift now fails in the normal `CI` workflow rather than only in scoped local acceptance. A second 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-new-shell-01-front-door-authority-fixture-followup.md` then centralized the required root/front-door wording lock into `scripts/lib/front-door-boundary-authority.mjs`, which is now consumed by both `scripts/check-shell-boundary-anti-drift.mjs` and `packages/hep-mcp/tests/docs/docToolDrift.test.ts` so those enforcement surfaces no longer maintain parallel string copies. A third 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-pipeline-a-public-literature-gap-retirement.md` then retired public `hepar literature-gap` from the installable legacy shell while keeping the internal full parser path for maintainer/eval/test usage, and updated the shared front-door wording fixture plus root docs so no installable `hepar literature-gap` front door remains. A fourth 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-pipeline-a-public-computation-run-retirement.md` then retired public `hepar run --workflow-id computation` from the installable legacy shell while keeping the internal full parser computation path for maintainer/eval/test usage, and updated the shared front-door wording fixture plus root docs so the installable `hepar` public shell now exposes neither `computation` nor `literature-gap`. A fifth 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-new-shell-01-first-touch-docs-front-door-followup.md` then extended the same shared wording fixture to first-touch docs: `docs/QUICKSTART.md` now frames itself as a domain-pack quickstart beneath the generic `autoresearch` front door, while `docs/TESTING_GUIDE.md` now explicitly distinguishes `autoresearch` as the generic lifecycle + workflow-plan front door from `@autoresearch/hep-mcp` as the current strongest domain MCP front door and demotes the internal `literature-gap` compatibility command to maintainer/eval/regression-only wording. A sixth 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-package-legacy-first-touch-docs-rebaseline.md` then pushed the same boundary down into package-level first-touch docs: `packages/hep-autoresearch/README.md`, `README.zh.md`, `docs/INDEX.md`, `docs/BEGINNER_TUTORIAL.md`, `docs/BEGINNER_TUTORIAL.zh.md`, `docs/WORKFLOWS.md`, and `docs/ORCHESTRATOR_INTERACTION.zh.md` now send readers back to repo-root front-door docs for current product truth, relabel package docs as package-local legacy / maintainer-oriented material, and stop presenting the installable legacy shell as if public computation, `doctor`, `bridge`, or `literature-gap` were still normal public entrypoints. A seventh 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-next-batch-public-run-residue-retirement.md` plus `prompt-2026-04-07-next-batch-front-door-authority-map.md` then moved beyond wording-only cleanup into exact survivor classification: installable public `hepar run --workflow-id ...` now exposes no workflow ids at all; `paper_reviser`, `ingest`, `reproduce`, `revision`, `literature_survey_polish`, and `shell_adapter_smoke` are explicitly internal full-parser coverage only; `meta/front_door_authority_map_v1.json` plus `scripts/lib/front-door-authority-map.mjs` classify `autoresearch_cli`, `hepar_public_shell`, `hepar_internal_full_parser`, and `orchestrator_mcp_tools_spec` as distinct surfaces; and `packages/hep-autoresearch/docs/BEGINNER_TUTORIAL*.md` plus `docs/ORCHESTRATOR_INTERACTION*.md` no longer teach retired public `ingest` / `reproduce` / `revision` paths. An eighth 2026-04-07 follow-up under `meta/docs/prompts/prompt-2026-04-07-hepar-public-shell-support-surface-retirement.md` then removed the remaining public support/operator verbs from the installable shell entirely. A ninth same-day delete-first follow-up under `meta/docs/prompts/prompt-2026-04-07-pipeline-a-delete-first-internal-support-wrappers.md` then finished the first parser-level contraction step: `approvals`, `report`, `logs`, `context`, `smoke-test`, `propose`, `skill-propose`, and `migrate` are now deleted from the parser rather than merely living as internal full-parser residue, while only `method-design`, `run-card`, and `branch` remain in that support-command bucket. These checks lock the continuously enforced truth that root docs must keep `autoresearch` as the generic lifecycle + workflow-plan front door, package-level first-touch docs must route current truth back to the repo-root front door rather than acting as a second product entrypoint, native TS `autoresearch workflow-plan` remains the recommended stateful launcher-backed literature entrypoint on initialized external project roots, lower-level Python workflow consumers remain lower-level or maintainer-only paths, the installable legacy `hepar` public shell now exposes exactly one command (`run`) with no public workflow ids or public support/operator siblings, deleted support wrappers stay deleted, HEP stays below root product identity, premature shell/gateway/frontend packages remain forbidden, `packages/shared` stays below provider-owned authority, `packages/orchestrator` stays below shell/UI/app authority, and hep-mcp must consume `@autoresearch/shared` / `@autoresearch/orchestrator` exports instead of redefining generic orchestrator authority locally. The 2026-04-06 follow-up corrected real drift in `README.md`, `docs/README_zh.md`, `docs/PROJECT_STATUS.md`, `docs/ARCHITECTURE.md`, and `docs/TOOL_CATEGORIES.md`; the earlier claim that no front-door wording fix was needed should be treated as superseded by this amended closeout note. A second 2026-04-06 follow-up then tightened the same truth further: docs, drift guards, and CLI-facing examples now explicitly surface the `autoresearch init` precondition plus `.autoresearch/state.json#/plan` / `.autoresearch/plan.md` persistence semantics for `autoresearch workflow-plan`, while the CLI implementation itself gained `plan_id` parity and direct test coverage for init fail-closed, active-run replacement rejection, and fallback `run_id` derivation. Acceptance passed on the closeout worktree: `git diff --check`; `node scripts/check-shell-boundary-anti-drift.mjs`; `pnpm --filter @autoresearch/shared test -- src/__tests__/package-boundary-authority.test.ts`; `pnpm --filter @autoresearch/orchestrator test -- tests/package-boundary.test.ts`; `pnpm --filter @autoresearch/hep-mcp test -- tests/contracts/sharedOrchestratorPackageExports.test.ts`; `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`; `node scripts/check-orchestrator-package-freshness.mjs`; `pnpm --filter @autoresearch/orchestrator build`; `pnpm --filter @autoresearch/hep-mcp build`; plus the later front-door checks `pnpm --filter @autoresearch/orchestrator test -- tests/autoresearch-cli.test.ts` and `pnpm --filter @autoresearch/orchestrator build`, the earlier package-doc follow-up check `python3 -m pytest -q packages/hep-autoresearch/tests/test_public_cli_surface.py`, the run-surface follow-up checks `python3 -m pytest -q packages/hep-autoresearch/tests/test_public_cli_surface.py packages/hep-autoresearch/tests/test_paper_reviser_workflow.py packages/hep-autoresearch/tests/test_adapter_gate_resolution_cli.py`, `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`, `pnpm --filter @autoresearch/orchestrator test -- tests/autoresearch-cli.test.ts`, and `node scripts/check-shell-boundary-anti-drift.mjs`, and the current support-surface retirement checks `python3 -m pytest packages/hep-autoresearch/tests/test_public_cli_surface.py -q`, `pnpm --filter @autoresearch/hep-mcp test -- tests/docs/docToolDrift.test.ts`, and `node scripts/check-shell-boundary-anti-drift.mjs`. The delete-first wrapper-removal acceptance adds `python3 -m pytest -q packages/hep-autoresearch/tests/test_approval_packet.py packages/hep-autoresearch/tests/test_report_renderer.py packages/hep-autoresearch/tests/test_evolution_proposal.py packages/hep-autoresearch/tests/test_evolution_trigger.py packages/hep-autoresearch/tests/test_migrate.py packages/hep-autoresearch/tests/test_mcp_doctor_and_bridge_cli.py`. Formal review then converged with 0 blocking via `Opus = CONVERGED_WITH_AMENDMENTS`, `Gemini-3.1-Pro-Preview` same-model embedded-source isolated-review rerun = `CONVERGED_WITH_AMENDMENTS`, and `OpenCode(zhipuai-coding-plan/glm-5)` same-model embedded-source rerun = `CONVERGED_WITH_AMENDMENTS`; the root/front-door follow-up then re-ran formal review on the updated front-door truth and reached 0 blocking with `Opus` R3 = `CONVERGED`, `Gemini-3.1-Pro-Preview` R3 = `CONVERGED`, and `OpenCode(zhipuai-coding-plan/glm-5.1)` = `CONVERGED` after a same-model verdict-normalization rerun. The current package-doc follow-up then also converged at 0 blocking after runner-path normalization reruns: the initial `Opus` review-mode path returned a non-archivable prompt/tool-result error, the initial `Gemini(auto)` review-mode path failed in `run_gemini.sh` proxy-env export logic, and the initial `OpenCode(zhipuai-coding-plan/glm-5.1)` workspace path produced no usable final artifact inside the review window; same-model embedded-source reruns for all three reviewers then returned `CONVERGED` with `blocking_issues = []`. The next real follow-up is now narrower and source-grounded: surviving internal full-parser residues (`doctor`, `bridge`, `literature-gap`, `paper_reviser`, `method-design`, `run-card`, and `branch`) need delete-vs-keep contract rebaseline and eventual removal, rather than another public-shell census sweep. The only remaining adopted amendments in the earlier follow-up were comment-level robustness clarifications for exact substring and exact directory-name matching.
