...
> **版本**: 1.9.26-draft (v1.9.25 + next-lane prompt bundle)
> **日期**: 2026-04-07
> **基线**: v1.9.23-draft
...
> - 新增 checked-in implementation prompt `meta/docs/prompts/prompt-2026-04-07-cp-obj-01c-delegated-runtime-session-turn-projection.md`，把 `CP-OBJ-01C` 明确收束为：共享 delegated runtime session/turn projection helper、`executeDelegatedAgentRuntime` 与 `runtime-diagnostics-bridge` 消费同一 projection seam、以及 `TeamAssignmentSession` 仅持久化最小稳定 lineage/terminal subset，而不是扩大成 transcript/job/public API redesign
> - 新增 checked-in implementation prompt `meta/docs/prompts/prompt-2026-04-07-docs-frontdoor-01-root-first-touch-ordering.md`，把 `DOCS-FRONTDOOR-01` 收束为 root `README.md` / `docs/README_zh.md` 的 generic-first first-touch ordering/prominence cleanup，并补齐 `scripts/check-shell-boundary-anti-drift.mjs` / `packages/hep-mcp/tests/docs/docToolDrift.test.ts` 对 order/prominence 的 fail-closed guard；`docs/QUICKSTART.md`、`docs/ARCHITECTURE.md`、`docs/PROJECT_STATUS.md` 当前更多是审计面而非主改面
>
> **v1.9.25 Changelog**:
