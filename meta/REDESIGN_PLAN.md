...
> **版本**: 1.9.43-draft (v1.9.42 + idea-engine contract/data-root authority cleanup)
> **日期**: 2026-04-07
> **基线**: v1.9.27-draft
...
> - the same slice retires the remaining public support verbs from the installable shell without reopening generic authority: `approvals`, `report`, `logs`, `context`, `smoke-test`, `method-design`, `propose`, `skill-propose`, `run-card`, `branch`, and `migrate` now live only on the internal full parser for maintainer/eval/regression coverage; `meta/front_door_authority_map_v1.json`, `scripts/lib/front-door-authority-map.mjs`, public CLI tests, and the shared boundary wording fixture now all lock that exact public-shell inventory across first-touch package docs plus the lingering `COMPUTATION*`, `EXAMPLES*`, `C2_method_design`, and `EVOLUTION.zh.md` doc surfaces
...
> - immediate next seams therefore shift from “clear the last public run workflow” to the next real closure queue: retire/rehome residual public authoring/support surfaces (`method-design`, `run-card`, `branch`, `smoke-test`, `migrate`, and the remaining support verbs), continue the later `idea-engine` asset/contract authority cleanup, and only then descend into the deeper runtime structural batch `DelegatedRuntimeHandleV1 -> RuntimePermissionProfileV1 -> DelegatedRuntimeTransport`
>
> **v1.9.40 Changelog**:
...
> **Rebaseline (2026-03-29, source audit; 2026-04-07 public-shell update)**: Python legacy CLI wiring for `approvals show --run-id <RID> --gate <A?> --format short|full|json` still exists, with tests covering empty/no-match/error paths plus manual smoke on all three output modes. The 2026-04-07 public-support retirement then removed `approvals` from the installable `hepar` public shell, so this surface now survives only on the internal full parser for maintainer/eval/regression coverage. TS orchestrator currently offers read/list consumers (`run-read-model.ts`, `approval.ts`) rather than a direct front-door `show` CLI replacement.
...
| 文件 | 修改内容 |
|---|---|
| `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` | internal full-parser `approvals show --run-id <RID> --gate <A?> --format short|full|json`；默认 `short`，终端打印 `packet_short.md`；`full` 打印 `packet.md`；`json` 输出 `approval_packet_v1.json` 到 stdout |
...
| `packages/hep-autoresearch/src/hep_autoresearch/orchestrator_cli.py` | internal full-parser `report render --run-ids <RID,...> --out <md|tex> [--output-path <path>]` |
...
**验收检查点**:
- [x] internal full-parser `report render --run-ids run_abc --out md` 生成自包含 Markdown 报告（installable public shell 已于 2026-04-07 退役该命令）
...
| internal full parser / canonical lifecycle | internal full-parser `approvals show` 默认打印 packet_short 到终端（不再仅显示 metadata）；批准动作本身走 canonical `autoresearch approve` |
...
- 完整 packet: internal full-parser `approvals show --run-id run_abc --gate A3 --format full`
```
...
- [x] internal full-parser `approvals show` 默认打印 packet_short 到终端
