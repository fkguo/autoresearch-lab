...
> **版本**: 1.9.41-draft (v1.9.40 + projection guard and public hepar run retirement)
> **日期**: 2026-04-07
> **基线**: v1.9.27-draft
...
- **缺失的显式 closeout 计划 (2026-03-24 planning completeness correction)**: 当前 repo 仍缺两个单独 bounded retirement slices，不能只停留在“Phase 4+ 退役”的高层口径：
  - `Pipeline A run-surface repoint / parity / delete`: 收口当前剩余的 Pipeline A Python delegation（installable public shell 现已进一步收窄：public `run --workflow-id computation` 已从 `hepar` / `hep-autoresearch` 退到 native TS `autoresearch` front door，`doctor` / `bridge` / `literature-gap` 已从 `hepar` / `hep-autoresearch` public surface 退到 internal full parser，仅 residual non-computation `run` workflows 与相邻 workflow/support commands 仍公开）；一旦 TS surface 通过验收，直接删除 `hep-autoresearch` / `hepar`
