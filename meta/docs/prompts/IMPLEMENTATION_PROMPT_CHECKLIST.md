...
实现 prompt 默认必须包含正式 `review-swarm` 收尾，且审核必须深入而非蜻蜓点水。默认 reviewer 固定为 `Opus` + `Gemini-3.1-Pro-Preview` + `OpenCode(zhipuai-coding-plan/glm-5)`；其中 `Gemini-3.1-Pro-Preview` 是 reviewer seat / 显示名，实际 `gemini` CLI model alias 固定写作 `gemini-3.1-pro-preview`。若其中任一模型本地不可用，必须记录失败原因并由人类明确确认 fallback reviewer。至少要求 reviewer：
...
5. 若本轮触及 public/package/CLI/workflow surface 或默认入口，检查仍在描述该 surface 的 front-door docs / public-surface locks 是否与当前实现一致；遗漏 live contradictory docs/tests 不能按“文档噪音”忽略，默认记为 `packet assumption breach`。
6. 对 shared/canonical authority 迁移类任务，检查 authority completeness，而不只是看命名或局部 diff：至少核对 authority map -> concrete artifact/template、artifact/template -> authority map、是否仍有 inline duplicate authority、以及 shared entrypoint / canonical acceptance 是否真的通过。
7. 检查 scope discipline，确认未顺手拉入 lane 外工作；但 shared entrypoint 或 canonical acceptance failure 默认先视为 packet assumption breach，而不是自动降级为 lane 外 debt，除非 reviewer 明确给出反证。
8. 对每个 blocking issue / amendment 给出文件级或测试级证据。
...
- 处理 reviewer failure 时，优先 same-model rerun；若 agentic/live file-read 路径不稳定，可改用更宽的 embedded-source rerun packet，但不得缩回 diff-only。只要 reviewer 仍在正常运行且有望产出 verdict，就不要主动终止它。对 `Gemini-3.1-Pro-Preview`，若问题集中在 agentic file-read / MCP discovery 路径，默认先做 same-model embedded-source rerun，不要改 reviewer seat，也不要把显示名当成 CLI alias 重写。
...
4. `Review-Swarm`：写明 mandatory reviewers（默认 `Opus` + `Gemini-3.1-Pro-Preview` + `OpenCode(zhipuai-coding-plan/glm-5)`；其中 Gemini CLI alias 固定为 `gemini-3.1-pro-preview`）、深审要求、收敛标准，以及 reviewer 必须显式回答“packet 对 blocker / debt / out-of-scope 的分类是否成立”
5. `Front-door Surface Audit`：若本批触及 public/package/CLI/workflow surface、默认入口或 lifecycle truth，prompt 必须显式列出通过搜索/枚举发现的 front-door docs / locks、哪些被纳入本轮 review、以及哪些被检查后判定不适用；遗漏这些 live surfaces 后再被 review 打回，默认按 packet assumption breach 处理
6. `Reviewer failure handling`：写明 runner/MCP/discovery 噪音本身不算 reviewer failure；只有无可用源码级 verdict 才算失败。默认先等待正常运行中的 reviewer，自然完成后再判断；若确实失败，优先 same-model rerun，必要时 widened embedded-source rerun，不得退化为 diff-only。对 Gemini 再额外写明：若问题集中在 agentic file-read / MCP discovery 路径，优先 same-model embedded-source rerun
...
7. `Self-Review`：写明 agent 自审也是 mandatory gate，且需绑定代码 / GitNexus / eval / scope 证据，并显式复核 packet assumptions 是否被推翻
8. `Authority completeness`：若任务涉及 shared/canonical authority 迁移，prompt 必须明确要求 `map -> artifact`、`artifact -> map`、`no inline duplicate authority`、`shared entrypoint acceptance` 四项检查
9. `交付后必须同步`：至少写明 tracker / `AGENTS.md` 必更，且 tracker 必须与当前代码/测试事实对齐；`architecture-decisions` 与 `REDESIGN_PLAN` 何时需要更新、何时明确不更新；对 `REDESIGN_PLAN` 需显式要求检查进度复选框、batch 表、完成态标题、当前现状、验收勾选与统计是否因本轮 closeout 而过时，并确认其与 tracker 和代码事实一致；以及 amendments / deferred 的持久 SSOT 去向
10. `生命周期状态同步`：若本批改变了某个 package / CLI / pipeline surface 的当前/过渡/退役语义，prompt 必须明确要求同步更新相关 authoritative docs 与仍把它当作当前默认入口的用户文档，并区分“现在还能用”与“长期应退役/改挂载”的表述
11. `版本控制门禁`：说明 commit/push 只有在收敛后且已获授权时才允许；若本批对应的 canonical prompt 文件已在当前 `worktree` 存在但尚未入库，必须与同批实现同次 commit 提交，除非人类明确排除或该文件并非本批 canonical prompt
12. `Post-closeout merge / cleanup`：若人类授权合入 `main` 并清理 batch worktree，说明 merge 只可在 completion + commit gate 满足后执行，且清理前必须先完成 Serena memory / SOTA archive 迁移门禁；若 merge 冲突或边界不明，必须停止而非半自动推进
13. `SOTA preflight / archive`：写明 canonical archive path（默认 `~/.autoresearch-lab-dev/sota-preflight/...`）以及当前 `worktree` 副本 / 指针路径
