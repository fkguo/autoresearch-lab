...
- 处理 reviewer failure 时，优先 same-model rerun；若 agentic/live file-read 路径不稳定，可改用更宽的 embedded-source rerun packet，但不得缩回 diff-only。只要 reviewer 仍在正常运行且有望产出 verdict，就不要主动终止它。
...
6. `Reviewer failure handling`：写明 runner/MCP/discovery 噪音本身不算 reviewer failure；只有无可用源码级 verdict 才算失败。默认先等待正常运行中的 reviewer，自然完成后再判断；若确实失败，优先 same-model rerun，必要时 widened embedded-source rerun，不得退化为 diff-only
