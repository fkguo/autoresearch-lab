...
- **NEW-COMP-01** (`meta/docs/computation-mcp-design.md`): 找到 NEW-COMP-01 的三个 `[ ]` 验收项并勾掉
...
NEW-COMP-01 (已完成) 定义了 `hep_run_ingest_skill_artifacts` 工具规格 (见 `meta/docs/computation-mcp-design.md`)。
...
输入 schema (参照 `meta/docs/computation-mcp-design.md`):
```typescript
{
  run_id: z.string(),
