# research-writer — M8-r1 Review Packet (distill consensus vs disagreements)

## Milestone goal
Add a **deterministic**, auditable distillation step that summarizes:
- which discussion-logic patterns are consistently identified by *both* models, and
- where the two models systematically diverge,
across a completed discussion-logic run directory (`packs/*/{claude,gemini}.md`).

This is intended to reduce agent/human load when scaling from N=10 → O(10^2–10^3) exemplar papers.

## Acceptance criteria
- New script exists: `scripts/bin/distill_discussion_logic.py`
  - Input: a discussion-logic run dir containing `packs/*/{claude,gemini}.md`
  - Output (written under `<run_dir>/distill/`):
    - `CONSENSUS.md` (patterns supported by both models; with counts)
    - `DISAGREEMENTS.md` (systematic model divergences; with paper IDs)
    - `STATS.json` (counts + summary; SSOT for downstream tooling)
- Deterministic normalization rules are documented in the output (and in code).
- Offline smoke coverage exists:
  - uses `--stub-models` to generate a 2-paper run and verifies deterministic `distill/` outputs.
...
[smoke] discussion-logic distill: offline (2-paper stub corpus)
...
- project root: /private/var/folders/8q/y82gg9g93mxff1x7gx7w4pl40000gn/T/tmp.lWDDmGxRtQ/project
- tag: M2-smoke
- out: /private/var/folders/8q/y82gg9g93mxff1x7gx7w4pl40000gn/T/tmp.lWDDmGxRtQ/paper
...
[smoke] latexmk: warnings=1 overfull_hbox=0
[smoke] bibtex fixer: adds journal field
[smoke] double-backslash checker+fixer: markdown math only
[smoke] ok
```

### Real-run example (external; not committed)
The distiller is designed to run on an existing discussion-logic directory, e.g.:
...
This writes:
```text
/Users/fkg/Nutstore Files/Coding/research_writer_discussion_logic/prl_hep-ph_xdj_hxz_fy_jz_mpospelov/distill/CONSENSUS.md
/Users/fkg/Nutstore Files/Coding/research_writer_discussion_logic/prl_hep-ph_xdj_hxz_fy_jz_mpospelov/distill/DISAGREEMENTS.md
/Users/fkg/Nutstore Files/Coding/research_writer_discussion_logic/prl_hep-ph_xdj_hxz_fy_jz_mpospelov/distill/STATS.json
```

The reports summarize high-frequency consensus tags (e.g. mechanism/constraints, formalism/representation, diagnostics) and highlight systematic differences (e.g. Claude tends to emphasize explicit error-budget hierarchy more often).

Concrete output excerpt (from `CONSENSUS.md`, truncated):

```text
## Top consensus tags (papers where *both* models mention the tag)
| tag | both | either | both/dual | agreement(both/either) | claude_only | gemini_only | examples |
|---|---:|---:|---:|---:|---:|---:|---|
| `mechanism_constraint` | 92 | 96 | 0.958 | 0.958 | 0 | 4 | 0705.1553, 0709.3272, 0903.4680 |
| `approach_formalism` | 90 | 95 | 0.938 | 0.947 | 0 | 5 | 0705.1553, 0709.3272, 0903.4680 |
| `scheme_scale_conventions` | 89 | 95 | 0.927 | 0.937 | 2 | 4 | 0705.1553, 0903.4680, 1003.0482 |
| `diagnostics_limits_scaling` | 88 | 96 | 0.917 | 0.917 | 2 | 6 | 0705.1553, 0709.3272, 0903.4680 |
| `headline_numbers_uncertainty` | 88 | 96 | 0.917 | 0.917 | 2 | 6 | 0705.1553, 0709.3272, 0903.4680 |
```

Concrete output excerpt (from `DISAGREEMENTS.md`, truncated):

```text
## Tags with largest asymmetry (Claude-only vs Gemini-only presence)
| tag | claude_only | gemini_only | delta | examples_claude_only | examples_gemini_only |
|---|---:|---:|---:|---|---|
| `error_budget_hierarchy` | 43 | 5 | 38 | 0705.1553, 0903.4680, 1003.0482 | 1210.5792, 1901.04497, 2010.10774 |
| `triangulation_multiple_routes` | 39 | 6 | 33 | 0910.5022, 1101.3273, 1112.1061 | 1406.2332, 2202.10524, 2207.03378 |
| `shift_attribution` | 17 | 26 | 9 | 0910.5022, 1009.2141, 1106.0266 | 1003.0482, 1008.3543, 1205.6525 |
```

Concrete output excerpt (from `STATS.json`, truncated):
...
### Robustness note (repairability)
`scripts/bin/research_writer_learn_discussion_logic.py` now treats **empty/garbled** model outputs as missing (via `_model_output_ok`), so `--mode repair` can re-run them deterministically instead of silently counting a 1-byte newline file as “complete”.

### Source code (primary deliverable)
`scripts/bin/distill_discussion_logic.py`:
...
## Reviewer request
Return `VERDICT: READY` only if M8 meets the acceptance criteria, is deterministic/auditable, and does not violate the Skill vs Agent boundary (no auto-mutation of the playbook).
