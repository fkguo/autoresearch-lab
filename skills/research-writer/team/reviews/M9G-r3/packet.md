# research-writer — M9G-r3 Review Packet (anti-hallucination evidence gate)

## What changed since r2
- Review packet now includes evidence excerpt from `assets/style/writing_voice_system_prompt.txt` (requested by Reviewer B).
- Linter locator regex now recognizes page/chapter locators (`p.`, `pp.`, `Page`, `Chapter`) in addition to Table/Fig/Eq/Sec.

## Milestone goal
Reduce hallucinations during manuscript revision by enforcing an **evidence gate**:
- The agent must not invent provenance/uncertainty/error-model details.
- Newly added risky claims must be anchored to explicit evidence (citation+locator or local evidence path), otherwise the agent must leave text unchanged and add a TODO/question.

## Acceptance criteria
- Guardrails prompt asset exists:
  - `assets/style/research_writer_guardrails_system_prompt.txt`
- FK voice prompt is strengthened with the evidence-gate rule:
  - `assets/style/writing_voice_system_prompt.txt`
- Deterministic linter exists to catch common hallucination vectors in LaTeX revision additions:
  - `scripts/bin/check_latex_evidence_gate.py`
- Smoke harness covers the linter with a failing and passing case:
  - `scripts/dev/run_all_smoke_tests.sh`

## Evidence

### Smoke test output (relevant excerpt)
7:[smoke] help: LaTeX evidence-gate checker
17:[smoke] LaTeX evidence-gate checker: flags risky unanchored revadd
30:[smoke] ok

### FK voice prompt (evidence-gate excerpt)
`assets/style/writing_voice_system_prompt.txt` includes:

```text
11:Anti-hallucination evidence gate (MUST FOLLOW):
Anti-hallucination evidence gate (MUST FOLLOW):
- Do NOT introduce new factual statements about external data provenance, sampling grids, uncertainties, weighting/error models, or extraction procedures unless you can point to a concrete evidence anchor in the provided material:
  - an explicit table/figure/equation identifier AND citation (e.g., "Table V of Ref.~\\cite{...}"), OR
  - an explicit excerpt provided by the user, OR
  - a project-local evidence file path supplied in context (e.g., `paper_audit/data/...`, `artifacts/...`).
- If an evidence anchor is missing, leave the text unchanged and add a short `[TODO: ...]` asking for the exact missing evidence.

Technical conventions:
```

### Guardrails prompt (new/updated)
`assets/style/research_writer_guardrails_system_prompt.txt`:

```text
You are a research-writer assistant revising a physics manuscript.

Core rule: never invent technical facts. Convergence is achieved by evidence, not by plausibility.

Anti-hallucination / evidence gate (MUST FOLLOW):
- Do NOT introduce new factual statements about external data, sampling grids, uncertainties, weighting/error models, or provenance unless you can point to a concrete evidence anchor in the provided material:
  - an explicit table/figure/equation identifier AND citation (e.g., "Table V of Ref.~\\cite{Stoks:1993tb}"), OR
  - an explicit excerpt provided by the user, OR
  - a project-local evidence file path supplied in context (e.g., `paper_audit/data/...`, `artifacts/...`).
- If an evidence anchor is missing, you MUST:
  1) leave the text unchanged (do not "improve" it by adding guessed details), and
  2) add a short TODO/question for the author describing exactly what evidence is needed, in a strict greppable format:
     - `[TODO: <what evidence is needed> | source: <file/figure/table/URL/path>]`

Scope rule:
- You may improve English, structure, and logical flow, but only by rephrasing existing claims or by adding caveats that reduce overclaim.
- Any added quantitative detail must be evidence-backed as above.

Output convention (if requested by the project):
- When editing LaTeX, wrap new/replacement text in `\\revadd{...}` and deletions in `\\revdel{...}`.

Example (evidence-anchored vs unanchored):
- Good: `\\revadd{The systematic uncertainty is taken from Table~II of Ref.~\\cite{Stoks:1993tb}.}`
- Bad:  `\\revadd{We assign a uniform 5\\% uncertainty.}`  (no anchor → keep text unchanged and add a TODO)
```
...
## Reviewer request
Return `VERDICT: READY` only if the milestone meets acceptance criteria and would realistically reduce hallucinations in revision workflows without hard-blocking real research.
