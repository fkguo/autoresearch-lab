# research-writer — M0-r4 Review Packet (physics discussion logic playbook + offline corpus fixture)

## Milestone goal
Integrate general physics discussion logic (argument flow, diagnostics, uncertainty narration) distilled from exemplar papers into the skill's drafting guidance, and strengthen offline smoke coverage for the INSPIRE→arXiv corpus fetcher.

## Acceptance criteria
- `assets/style/physics_discussion_logic_playbook.md` exists and provides a reusable mind-map template + checklist + prompt template for per-paper argument maps.
- `assets/style/style_profile.md` and `assets/style/writing_voice_system_prompt.txt` reference the playbook and keep skepticism + auditability requirements.
- `assets/style/prl_style_corpus.md` is framed as exemplar *discussion-logic* learning (not superficial PRL formatting).
- Smoke tests exercise the corpus fetcher in deterministic offline mode (fixture INSPIRE JSON + fixture arXiv tar), including unsafe-path rejection + trace logging.
- No files are modified in the external Overleaf corpus.

## Summary of changes
- Added `assets/style/physics_discussion_logic_playbook.md` and wired it into the style profile + drafting prompt.
- Reframed the PRL corpus doc as exemplar discussion-logic learning and updated templates to nudge the Discussion flow.
- Extended smoke tests with an offline extraction fixture for `fetch_prl_style_corpus.py` (no network).
...
- project root: /private/var/folders/8q/y82gg9g93mxff1x7gx7w4pl40000gn/T/tmp.WLlN6pkjR4/project
- tag: M2-smoke
- out: /private/var/folders/8q/y82gg9g93mxff1x7gx7w4pl40000gn/T/tmp.WLlN6pkjR4/paper
...
Status: in progress (requires new dual-model review after updates).
...
### 6) Distill discussion logic (mind maps → playbook)
...
- For Results/Discussion, default to: bottom line → mechanism → diagnostics → comparison → limitations/outlook (see `assets/style/physics_discussion_logic_playbook.md`).
