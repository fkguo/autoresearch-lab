# research-writer — M9G-r1 Review Packet (anti-hallucination evidence gate)
...
- Deterministic linter exists to catch common hallucination vectors in LaTeX revision additions:
  ...
  - Default scans `\\revadd{...}` blocks and flags risky provenance/uncertainty/error-model claims without:
    - `Table/Fig/Eq/Sec/...` **and** `\\cite{...}`, OR
...
- No violation of Skill vs Agent boundary:
  - tooling adds checks/prompt templates; it does not auto-mutate the playbook based on model outputs.
...
```text
[smoke] help: LaTeX evidence-gate checker
...
[smoke] LaTeX evidence-gate checker: flags risky unanchored revadd
...
[smoke] ok
```

### Guardrails prompt (new)
`assets/style/research_writer_guardrails_system_prompt.txt`:
...
- If an evidence anchor is missing, you MUST:
  ...
  2) add a short TODO/question for the author describing exactly what evidence is needed.
...
### FK voice prompt (strengthened excerpt)
`assets/style/writing_voice_system_prompt.txt` now includes:
...
- Do NOT introduce new factual statements about external data provenance, sampling grids, uncertainties, weighting/error models, or extraction procedures unless you can point to a concrete evidence anchor in the provided material:
  - an explicit table/figure/equation identifier AND citation ..., OR
  - an explicit excerpt provided by the user, OR
  - a project-local evidence file path supplied in context ...
...
_RE_LOCATOR = re.compile(r"\b(Table|Tab\.|Figure|Fig\.|Equation|Eq\.|Section|Sec\.|Appendix)\b", flags=re.I)
